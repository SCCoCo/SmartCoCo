...
// File contracts/adapters/VesperWETHVaultAdapter.sol
...
contract VesperWETHVaultAdapter is IWETHVaultAdapterV2 {
  ...
  function token() external view override returns (IWETH) {
    return IWETH(vault.token());
  }
  ...
  function withdraw(address _recipient, uint256 _amount, bool _isHarvest) external override onlyAdmin {
    ...
    IWETH vaultToken = IWETH(vault.token());
    ...
    if(_isHarvest && vspRewardsAmount > 0){
      address[] memory _pathVesper = new address[](2);
      _pathVesper[0] = address(vesperToken);
      _pathVesper[1] = address(vaultToken);
  ...
  function updateApproval() public {
    ...
    IWETH(_token).approve(address(vault), uint256(-1));
