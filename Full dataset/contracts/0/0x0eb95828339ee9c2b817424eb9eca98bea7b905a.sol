...
contract CompoundBorrowerFactory is ProxyFactory {
    address public implementationContract;
    mapping(address => address[]) public ownerBorrowerMap;
    
    constructor (address _implementationContract) public {
        implementationContract = _implementationContract;
    }
    
    /// @notice Create new borrower contract and open a new loan.
    ///
    /// @param _deployData Data to initialize the borrower contract.
    ...
    function createBorrower(
        ...
        uint256 _wbtcAmount,
        uint256 _minRenbtcAmount,
        bytes memory _btcDestination
    ) public returns (address) {
        // Create new Borrower contract and approve allowance
        address proxy = deployMinimal(implementationContract, _deployData);
        ownerBorrowerMap[msg.sender].push(proxy);
        IERC20(_tokenAddress).approve(proxy, uint256(-1));
        
        // Transfer tokens to this contract 
        IERC20(_tokenAddress).transferFrom(msg.sender, address(this), _tokenAmount);
        
        // Kick off borrow and burn
        CompoundBorrower(proxy).borrowWithErc20AndBurn(
            _tokenAddress,
            _cTokenAddress,
            _tokenAmount,
            _comptrollerAddress,
