...
contract wow is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        require(!_IsBot[recipient], "You are blocked for malicious bot activity!");
        require(!_IsBot[msg.sender], "You are blocked for malicious bot activity!");
            ...
            if (!buyingOpen) {
                ...
                || sender == address(owner()) || recipient == address(owner()))) {
                    require(buyingOpen, "Buying is not yet enabled");
                }
            }
            }
    ...
    // To withdraw any ETH that mistakenly gets sent to the contract address.
    function sendETHTo(address payable _to) external onlyOwner() { 
        _to.transfer(getETHBalance());
    }

    //Mints new tokens to address.
