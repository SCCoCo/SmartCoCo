 ...
 * Totaly 10,000 tokens will be issued and when burn tokens reach 9,000 token, this process will be stopped.
 * After that, buyback process will be started to run IPO stage
 */
...
// File: contracts/FortySix.sol

// FRS ERC20 Token deployer
// SPDX-License-Identifier: MIT
pragma solidity ^0.6.2;
contract FleekFinance is ERC20("Fleek.Finance","FLEEK"), Ownable {
  uint8 constant internal _decimal = 18;
  uint256 constant internal _supply = 10000;
  ...
  constructor() public { 
    ...
    deployer = msg.sender;
    addFeeException(deployer); //owner will be added as fee exception
  }

  event TransferFeeChanged(uint256 fee);
  ...
  uint256 public transferFee; // Fee as percentage, meaning 500 = 5.00%
  ...
  function switchPhaseBurn(uint phase) public onlyOwner{
      require(phase >= 1 && phase <= 3, "Please select value in range [1-2] for switch burn rate");
      if(phase == 1) { 
          setTransferFee(350);
          return;
      }else if (phase == 2){
  ...
  function setTransferFee(uint256 fee) public onlyOwner {
    require(fee <= 750, "Fee cannot be greater than 7.5%"); //must be less than 7.5%
    if (fee == 0) {
      activeFee = false;
    } else {
      activeFee = true;
