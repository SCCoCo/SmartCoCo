...
contract Unicorn {
    ...
    uint public totalSupply = 5_000_000e18; // 5 million Unic
    ...
    function setMinter(address minter_) external {
        require(msg.sender == minter, "Unic::setMinter: only the minter can change the minter address");
    ...
    function mint(address dst, uint rawAmount) external {
        require(msg.sender == minter, "Unic::mint: only the minter can mint");
        require(block.timestamp >= mintingAllowedAfter, "Unic::mint: minting not allowed yet");
        ...
        require(amount <= SafeMath.div(SafeMath.mul(totalSupply, mintCap), 100), "Unic::mint: exceeded mint cap");
        totalSupply = safe96(SafeMath.add(totalSupply, amount), "Unic::mint: totalSupply exceeds 96 bits");

        // transfer the amount to the recipient
        balances[dst] = add96(balances[dst], amount, "Unic::mint: transfer amount overflows");
