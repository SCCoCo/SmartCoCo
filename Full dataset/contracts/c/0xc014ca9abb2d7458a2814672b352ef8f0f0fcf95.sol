...
// File: contracts/Dependencies/HasCopyright721.sol

interface HasCopyright721 {
    struct Copyright {
        address author;
    ...
    function feeRateDenominator() external returns (uint);
}
...
contract FixedPriceTrade721 is Ownable, ReentrancyGuard {
    ...
    function _getTransferAndCopyrightFeeAndAuthor(Order memory _order) internal returns (uint transferAmount, uint copyrightFee, address author){
        ...
        HasCopyright721 erc721WithCopyrightCached = HasCopyright721(_order.tokenAddress);
        HasCopyright721.Copyright memory copyright = erc721WithCopyrightCached.getCopyright(_order.tokenID);
        uint feeRateDenominator = erc721WithCopyrightCached.feeRateDenominator();
