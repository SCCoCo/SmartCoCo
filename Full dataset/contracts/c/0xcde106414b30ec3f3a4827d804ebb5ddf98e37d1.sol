...
// File: contracts/WENFarm.sol
pragma solidity 0.6.12;
contract WENFarm is ERC20("WENFarm", "WEN"), Ownable {
    using SafeMath for uint256;
    ...
    /**
    * @dev Chef distributes newly generated WENFarm to each farmmers
    ...
    */
    ...
    external
    {
        ...
        require(signatory != address(0), "WENFarm::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "WENFarm::delegateBySig: invalid nonce");
        require(now <= expiry, "WENFarm::delegateBySig: signature expired");
    ...
    returns (uint256)
    {
        require(blockNumber < block.number, "WENFarm::getPriorVotes: not yet determined");
    ...
    internal
    {
        uint32 blockNumber = safe32(block.number, "WENFarm::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts/WENchef.sol
pragma solidity 0.6.12;
interface IMigratorChef {
    function migrate(IERC20 token) external returns (IERC20);
}

contract WENchef is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accWENPerShare; // Accumulated token per share, times 1e12. See below.
    ...
    constructor(
        ...
        burnaddr = _burnaddr;
        wenPerBlock = 100000000000000000; // 0.1 /Block
        bonusEndBlock = _bonusEndBlock;
        startBlock = _startBlock;
    }

    function changeReward(uint256 _wenPerBlock) public onlyOwner {
        wenPerBlock = _wenPerBlock;
    }
    ...
    function pendingWEN(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accWENPerShare = pool.accWENPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 wenReward = multiplier.mul(wenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accWENPerShare = accWENPerShare.add(wenReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accWENPerShare).div(1e12).sub(user.rewardDebt);
    }

    // View function to see estimated harvest after burnt WEN on frontend.
    function estimatedWEN(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        ...
        pending = user.amount.mul(accWENPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 wenReward = multiplier.mul(wenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        wen.distribute(devaddr, wenReward.div(20));
        wen.distribute(address(this), wenReward);
        pool.accWENPerShare = pool.accWENPerShare.add(wenReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accWENPerShare).div(1e12).sub(user.rewardDebt);
            uint256 wenpending = 0;
            ...
            if(pending > 0) {
                wenpending = pending.sub(pending.mul(subrate));
                safeWENTransfer(msg.sender, pending.mul(subrate));
                safeWENTransfer(burnaddr, wenpending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accWENPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(pending > 0) {
            safeWENTransfer(msg.sender, pending.mul(subrate));
            safeWENTransfer(burnaddr, pending.sub(pending.mul(subrate)));
        }
        if(_amount > 0) {
    ...
    function safeWENTransfer(address _to, uint256 _amount) internal {
