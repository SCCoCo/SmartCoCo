...
contract ShellMiner is OwnershipRole {
    ...
    function getUserProfit(address _user) public view returns (uint) {
        ...
        uint _profit = _blocks.mul(_miners);
        return (_profit);
    }

    function mining() external returns (uint) {
        uint contribution = matrix.getUserContribution(matrix.ids(_msgSender()));
        ...
        if (start > 0) {
            ...
            _restart(_msgSender(), contribution, current);
            token.mint(_msgSender(), profit);
            return (profit);
        } else {
            _initialize(_msgSender(), contribution, current);
            return (0);
        }
    }

    function _initialize(address _user, uint _contribution, uint _block) private {
        User memory user = User({
            miners: _contribution,
            startBlock: _block
        });
    ...
    function _restart(address _user, uint _contribution, uint _block) private {
        users[_user].miners = _contribution;
