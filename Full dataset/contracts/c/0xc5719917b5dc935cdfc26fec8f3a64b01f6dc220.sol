...
// File: contracts/Role/AdminRole.sol
...
/**
 * @title AdminRole
 * @dev Admins are responsible for assigning and removing ed accounts.
 */
contract AdminRole {
  ...
  function removeAdmin(address account) public onlyAdmin {
    require(_addresses.length > 1, "AdminRole should not be 0"); // 追加
  ...
  function _addAdmin(address account) internal {
    _admins.add(account);
    _addresses.push(account); // 追加
    emit AdminAdded(account);
  }

  function _removeAdmin(address account) internal {
    _admins.remove(account);
    _addresses = _addresses.remove(account); // 追加
    emit AdminRemoved(account);
  }
}

// File: contracts/Role/MinterRole.sol
...
/**
 * @title MinterRole
 * @dev Minters are responsible for assigning and removing ed accounts.
 */
contract MinterRole is AdminRole {
  ...
  function removeMinter(address account) public onlyAdmin {
    require(_addresses.length > 1, "MinterRole should not be 0"); // 追加
...
// File: contracts/Role/ModeratorRole.sol
...
/**
 * @title ModeratorRole
 * @dev Moderators are responsible for assigning and removing ed accounts.
 */
...
// File: contracts/CryptoCollectiveAsset.sol
