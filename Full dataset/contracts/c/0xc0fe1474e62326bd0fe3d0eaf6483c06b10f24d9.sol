...
// File contracts/QUAI_Controller.sol
...
contract QUAI_Controller is QuaiAccessControlDefended {
    ...
    /// @param _keeper An extra address that can call earn() to deposit tokens from a Vault into the associated active Strategy. Designed for use by a trusted bot in lieu of having this function publically callable.
    ...
    /// @notice The Vault for a given token or any of the permissioned roles can call earn() to deposit accumulated deposit funds from the Vault to the active Strategy
    ...
    /// @notice Set the Vault for a given want
    ...
    /// @notice Entire balance will be sent to corresponding Vault.
    ...
    /// @notice Deposit given token to strategy, converting it to the strategy's want first (if required).
    ...
    /// @notice Withdraw a given token from its corresponding strategy
