...
contract ETHDIAMOND {
    ...
    modifier checkExchangeOpen(uint256 _amountOfEthereum){
        if( exchangeClosed ){
            require(isInHelloDiamond_[msg.sender]);
    ...
    uint8 constant internal sellFee_ = 15;//15%
    uint8 constant internal transferFee_ = 10;
    uint8 constant internal devFee_ = 20; // 5%
    ...
    */
        ...
        public
    {
        ...
        uint256 _devPool = SafeMath.div(_ethereum,devFee_); //3%
    ...
    function helloDiamond(address _address, bool _status,uint8 _count)
      public
    {
      require(msg.sender == devAddress_);
        ...
        returns(uint256)
    {
        // data setup
        address _customerAddress = msg.sender;
        uint256 _undividedDividends = SafeMath.div(SafeMath.mul(_incomingEthereum, buyFee_), 100); //33%
