...
// File contracts/WaLUSDToken.sol
...
contract WaLUSDToken is AccessControl, ERC20("Wasabi LUSD", "waLUSD") {
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "waLUSD: Alchemist is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "waLUSD: Alchemist is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"waLUSD: Alchemist's ceiling was breached.");
    require(!paused[msg.sender], "waLUSD: user is currently paused.");
