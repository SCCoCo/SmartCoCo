...
// File: contracts/EliteStakingRewardsFactory.sol
...
contract EliteStakingRewardsFactory is Ownable {
    // immutables
    ...
    struct EliteStakingRewardsInfo {
        address stakingRewards;
        uint rewardAmount;
    }

    // rewards info by staking token
    mapping(address => EliteStakingRewardsInfo) public stakingRewardsInfoByStakingToken;

    constructor(
        address _rewardsToken,
        uint _stakingRewardsGenesis
    ) Ownable() public {
        require(_stakingRewardsGenesis >= block.timestamp, 'EliteStakingRewardsFactory::constructor: genesis too soon');
    ...
    function deploy(address stakingToken, uint rewardAmount) public onlyOwner {
        EliteStakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[stakingToken];
        require(info.stakingRewards == address(0), 'EliteStakingRewardsFactory::deploy: already deployed');

        info.stakingRewards = address(new EliteStakingRewards(/*_rewardsDistribution=*/ address(this), rewardsToken, stakingToken));
    ...
    function notifyRewardAmounts() public {
        require(stakingTokens.length > 0, 'EliteStakingRewardsFactory::notifyRewardAmounts: called before any deploys');
    ...
    function notifyRewardAmount(address stakingToken) public {
        require(block.timestamp >= stakingRewardsGenesis, 'EliteStakingRewardsFactory::notifyRewardAmount: not ready');

        EliteStakingRewardsInfo storage info = stakingRewardsInfoByStakingToken[stakingToken];
        require(info.stakingRewards != address(0), 'EliteStakingRewardsFactory::notifyRewardAmount: not deployed');
        ...
        if (info.rewardAmount > 0) {
            ...
            require(
                ...
                'EliteStakingRewardsFactory::notifyRewardAmount: transfer failed'
            );
            EliteStakingRewards(info.stakingRewards).notifyRewardAmount(rewardAmount);
        }
    }
}