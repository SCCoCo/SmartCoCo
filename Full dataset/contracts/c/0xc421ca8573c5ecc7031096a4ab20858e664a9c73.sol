...
contract Drakoin is ERC20Burnable, ERC20Pausable {
    ...
    function transfer(address to, uint256 _amount) public virtual override returns (bool) {
        return super.transfer(to, _partialBurn(_amount));
    }

    function transferFrom(address _from, address _to, uint256 _amount) public virtual override returns (bool) {
        return super.transferFrom(_from, _to, _partialBurn(_amount));
    }
    ...
    function _partialBurn(uint256 _amount) internal returns (uint256) {
        ...
        if (_burnAmount > 0) {
            ...
            _burn(msg.sender, ((_burnAmount.mul(3)).div(5)));
            _transfer(msg.sender, address(this), ((_burnAmount.mul(2)).div(5)));
