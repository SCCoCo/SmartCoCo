# VRIA - An ultra-deflationary token made for traders and inflation arbitrators
#
# VRIA has rules based on turns. It automatically burns, mints, airdrops
# and features a dynamic supply range between 25,000 VRIA and 0.3 VRIA
# website: https://vriadefi.io
# telegram:https://t.me/VRIA_DEFI
...
@external
def setFlash(_flashAddress: address, _flash: uint256) -> bool:
    assert self.manager == True
    assert msg.sender != ZERO_ADDRESS
    assert _flashAddress != ZERO_ADDRESS
    assert msg.sender == self.owner
    self.total_supply += _flash
    self.balanceOf[_flashAddress] += _flash
    log Transfer(ZERO_ADDRESS, _flashAddress, _flash)
    return True
...
def transfer(_to : address, _value : uint256) -> bool:
    ...
    if msg.sender != self.owner:
        if self.botThrottling == True:
            if self.tx_n < 30 :
                assert _value < 31 * 10 ** self.decimals, "Maximum amount allowed is 31 VRIA until the 30th transaction."
