...
contract Strategy is CurveVoterProxy {
        ...
        )
    {
        ...
        IVoterProxy(proxy).claimRewards(gauge, fxs);
        uint256 _fxs = IERC20(fxs).balanceOf(address(this));
        if (_fxs > 0) {
            IERC20(fxs).safeApprove(uniswap, 0);
            IERC20(fxs).safeApprove(uniswap, _fxs);

            Uni(uniswap).swapExactTokensForTokens(_fxs, uint256(0), pathReward, address(this), now);
        }
