 ...
 * @author  Sunder.Team
 *
 * @dev     Contract for Sunder token with burn support
 */
...
// Sunder erc20 Token Contract.
contract Sunder is DeligateERC20, Ownable {
    uint256 private constant maxSupply      = 200 * 1e6 * 1e18;     // the total supply

    uint256 private idoSupply       = 5 * 1e6 * 1e18;       // pre-mine for ido
    address private idoAdmin        = msg.sender;
    
    uint256 private preMineSupply   = 85 * 1e6 * 1e18;      // pre-mine for locking
    ...
    constructor() public ERC20("Sunder Goverance Token", "Sunder") {
        _mint(idoAdmin, idoSupply);
        _mint(preMineAdmin, preMineSupply);
    }
    ...
    function addMinter(address _addMinter) public onlyOwner returns (bool) {
        require(_addMinter != address(0), "Alert: _addMinter is the zero address");
        
        return EnumerableSet.add(_minters, _addMinter);
    }

    function delMinter(address _delMinter) public onlyOwner returns (bool) {
        require(_delMinter != address(0), "Alert: _delMinter is the zero address");
    ...
    modifier onlyMinter() {
        require(isMinter(msg.sender), "Alert: caller is not the minter");
        _;
    }
}