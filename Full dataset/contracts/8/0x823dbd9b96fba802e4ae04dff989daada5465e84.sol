...
/*
    ...
    - Измение размера бонуса происходит функцией `setBonusPercent(_value)`, где `_value` - % начисляемых бонусов при покупке токенов, смена стоимости токена доступно только во время паузы администратору, после завер��ения Crowdsale функция становится недоступной
