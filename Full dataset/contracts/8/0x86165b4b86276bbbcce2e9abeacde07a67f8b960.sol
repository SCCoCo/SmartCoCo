...
contract HYA {
	...
	uint256 constant private Total_HYA = 10e22; 
	uint256 constant private Burn_Ratio = 1;   // 12% burn rate for each transaction
	uint256 constant private Burn_Threshold = 2;     
	uint256 constant private Freeze_Size = 1e20; 
	
	string constant public name = "Hyatt";
	string constant public symbol = "HYA";
	...
	constructor() public {
		...
		info.totalSupply = Total_HYA;
		info.users[msg.sender].balance = Total_HYA;
		emit Transfer(address(0x0), msg.sender, Total_HYA);
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		if (totalSupply() - _burnedAmount < Total_HYA * Burn_Threshold / 100 || isWhitelisted(_from)) {
			_burnedAmount = 0;
		}
