...
contract CoinPoolAlpha is PausableToken {
  ...
  modifier onlyWhenTransferEnabled() {
    ...
    if (!transferable) {
      ...
      require(msg.sender == owner || msg.sender == CPAWallet);
  ...
  constructor(address _CPAWallet) public StandardToken("CoinPoolAlpha Token", "CPA", 18) {



    require(_CPAWallet != address(0));

    CPAWallet = _CPAWallet;

    totalSupply_ = INITIAL_SUPPLY;

    balances[_CPAWallet] = totalSupply_;

    emit Transfer(address(0), _CPAWallet, totalSupply_);
