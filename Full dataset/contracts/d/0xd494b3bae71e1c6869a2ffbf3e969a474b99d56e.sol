...
interface AaveV2ProtocolDataProvider {
...
contract AaveV2Helpers is AaveHelpers {
    ...
    */
    ...
    function collateralData(AaveV2ProtocolDataProvider aave, address token) internal view returns(AaveTokenData memory) {
        AaveTokenData memory aaveTokenData;
        (
            ,
            aaveTokenData.ltv,
    ...
    public view returns (uint) {
        ...
        AaveV2ProtocolDataProvider aave = AaveV2ProtocolDataProvider(getAaveV2ProtocolDataProvider());
        (address atoken,,) = aave.getReserveTokensAddresses(token);
        if (atoken == address(0)) return 0;
        
        AaveTokenData memory aaveToken = collateralData(aave, token);
        AaveTokenData memory aaveEthToken = collateralData(aave, getWethAddress());
        if (!aaveToken.borrowEnabled) return 0;
