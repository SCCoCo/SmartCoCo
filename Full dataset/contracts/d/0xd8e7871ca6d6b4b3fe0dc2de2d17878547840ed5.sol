...
// ReapitToken with Governance.
contract ReapitToken is ERC20Burnable, Ownable {
	...
	/// @notice Creates `_amount` token to `_to`. Must only be called by the owner (REAPfarmer).
	...
	constructor() public ERC20('Reapit.IO', 'REAP') {}
	...
	function delegateBySig(
		...
		require(signatory != address(0), 'REAP::delegateBySig: invalid signature');
		require(nonce == nonces[signatory]++, 'HULK::delegateBySig: invalid nonce');
		require(now <= expiry, 'REAP::delegateBySig: signature expired');
		...
		returns (uint256)
	{
		require(
			...
			'REAP::getPriorVotes: not yet determined'
		);
	...
	function _delegate(address delegator, address delegatee) internal {
		...
		uint256 delegatorBalance = balanceOf(delegator); // balance of underlying REAP (not scaled);
	...
	function _writeCheckpoint(
		...
		uint32 blockNumber = safe32(
			block.number,
			'REAP::_writeCheckpoint: block number exceeds 32 bits'
		);
