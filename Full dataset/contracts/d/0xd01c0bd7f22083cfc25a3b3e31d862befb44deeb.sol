...
library Heap{ // default max-heap
  ...
  function insert(Data storage self, int128 priority) internal returns(Node){//¡Ì
  ...
  function extractMax(Data storage self) internal returns(Node){//¡Ì

    return _extract(self, ROOT_INDEX);

  }

  function extractById(Data storage self, int128 id) internal returns(Node){//¡Ì
  ...
  function _extract(Data storage self, uint i) private returns(Node){//¡Ì
  ...
  function _bubbleUp(Data storage self, Node memory n, uint i) private{//¡Ì
  ...
  function _insert(Data storage self, Node memory n, uint i) private{//¡Ì
