...
// HIGH RISK DEFLATIONARY FRIDAY PAMP
...
contract FridayPAMP is ERC20("FRIDAY PAMP", "yfiDAY"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (Chef).
    ...
    function _partialBurn(address sender, uint256 amount) internal returns (uint256) {
        uint256 burnAmount = (amount.mul(150)).div(1000);
        if (burnAmount > 0) {
