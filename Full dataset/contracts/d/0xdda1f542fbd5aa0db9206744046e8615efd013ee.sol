...
// Root file: contracts/NFTitsColletion.sol

pragma solidity >=0.8.0;

/**
 * @title NFTitsCollection - "Bikini Bottom" collection.
 */
contract NFTitsCollection is ERC721Enumerable, ReentrancyGuard, Ownable {
    using SafeMath for uint256;

    address public bank;
    uint256 public raisedEthers;
    ...
    event BankSet(address bankAccount);
    event RolledOver(bool status);

    constructor(address _bank)
    ERC721("CryptoNFTits", "NFTITS")
    {
        bank = _bank;
        emit BankSet(_bank);
    }
    ...
    receive() external payable {
        require(
            ...
            "Collection: smart contract call"
        );

        uint deposit = msg.value;

        require(deposit % tokenPrice == 0, "Collection: quantity with change");

        uint numberOfTokens = deposit.div(tokenPrice);

        require(
            numberOfTokens != 0 && numberOfTokens <= maxTokenPurchase,
            "Collection: incorrect token amount mint, exceeds max or min number"
        );
        require(
        ...
        if (bank != address(this)) {
            Address.sendValue(payable(bank), deposit);
        }

        raisedEthers += deposit;
    ...
    function mintTokens(uint numberOfTokens) public payable nonReentrant {
        ...
        require(tokenPrice.mul(numberOfTokens) == deposit,
    ...
    // @dev Withdraw ethereum from contract balance.
    ...
    // @dev Setup new max tokens amount, should be more then 10K.
    // Permission: only owner
    ...
    // Permission: only owner
    function setBank(address _account) external onlyOwner {
        require(_account != address(0), "Zero address set");

        bank = _account;
        emit BankSet(_account);
    }
