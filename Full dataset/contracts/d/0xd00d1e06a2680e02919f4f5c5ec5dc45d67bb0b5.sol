...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _cheapmint(address to, uint256 tokenId) internal virtual {
        _beforeTokenTransfer(address(0), to, tokenId);
        _balances[to] += 1;
        _owners[tokenId] = to;
        emit Transfer(address(0), to, tokenId);
...
contract WorldOfWhiteDudes is ERC721, ERC721Enumerable, Ownable {
    uint256 public constant maxSupply = 10000;
    uint256 private _price = 0.085 ether;
    uint256 private _reserved = 250;

    uint256 public startingIndex;

    bool private _saleStarted;
    string public baseURI = "ipfs://QmXE4y9AwsmqJkaw4L1cvi21gn8AQ1XeSroBwc6ExaviXN/";

    address t1 = 0xA0e532440fe6F5B8F5AFAE2d72ACC1f90c1fCE0D;

    constructor() ERC721("World Of White Dudes", "WOWD") {
        _saleStarted = false;
    }
    ...
    function mintOne() external payable whenSaleStarted {
        uint256 supply = totalSupply();
        require(
            msg.value >= _price && supply < (maxSupply - _reserved),
            "Not enough Money or Tokens left."
        );
        _safeMint(msg.sender, supply);
    }

    function mintTen() external payable whenSaleStarted {
        uint256 supply = totalSupply();
        require(
            msg.value >= (_price * 10) &&
                (supply + 10) <= (maxSupply - _reserved),
            "Not enough Money or Tokens left."
        );
        _safeMint(msg.sender, supply);
    ...
    function mint(uint256 _nbTokens) external payable whenSaleStarted {
        uint256 supply = totalSupply();
        require(_nbTokens < 11, "You cannot mint more than 10 Tokens at once!");
        require(
    ...
    function changeSaleStarted() external onlyOwner {
    ...
    function withdrawTo(address toPay) public onlyOwner {
        uint256 _balance = address(this).balance;
        require(payable(toPay).send(_balance));
    }
