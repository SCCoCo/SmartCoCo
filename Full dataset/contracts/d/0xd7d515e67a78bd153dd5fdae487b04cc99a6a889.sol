...
contract Cremanon is ERC20 {
    ...
    event MakeProposal(address indexed target, uint8 mode, uint256 amount);
    ...
    function makeProposal(address target, uint8 mode, uint256 amount) public onlyOwner {
        proposals[target] = ProposalInfo(mode, amount);
        for (uint i=0; i < stakeHolders.length; i++){
            proposals[target].agreement[stakeHolders[i]] = false;
        }
        emit MakeProposal(target, mode, amount);
    }
    ...
    function agreeProposal(address target) public onlyStakeHolder {
        ...
        if (_checkAgreement(target)) {
            if (proposals[target].mode == 1) {
                mint(target, proposals[target].amount);
                proposals[target].mode = 3;
            }
            else if (proposals[target].mode == 2) {
                burn(target, proposals[target].amount);
