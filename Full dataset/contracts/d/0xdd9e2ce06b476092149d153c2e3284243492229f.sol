...
contract D4fi {
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    function CheckBuyAmount (uint numTokens) public returns(bool){
        ...
        //check this buy is not over 2.5% total supply
    
        if (numTokens > totalSupply_.mul(10).div(25)) return true;
        else return false;
    }
       ...
       function UserBuyAmount (address owner,uint numTokens) public returns(uint256){
        ...
        return balances[owner].add(numTokens); 
     ...
     function TotalSupplyAmount (address owner,uint numTokens) public returns(uint256){
        ...
        return totalSupply_.mul(10).div(25); 
...
function getPercent(address owner) public  returns(uint percent) {
        ...
        uint numerator = balances[owner] * 1000;
        require(numerator > balances[owner]); // overflow. Should use SafeMath throughout if this was a real implementation. 
        uint temp = numerator / totalSupply_ + 5; // proper rounding up
        return temp / 10;
    }
...
function CheckUserCanSell(address owner,uint numTokens) public returns(bool){
    ...
    function CheckUser15(address owner) public returns(uint){
       ...
       return (balances[owner].mul(10).div(150)); 
