...
contract Niron is Context, Ownable, ERC20 {
    using SafeMath for uint256;
    ...
    uint256 public rewardDivider = 0;
    ...
    uint256 public minTokenBeforeSwap = 1000000 * 10**18;
    ...
    constructor() ERC20("Niron","NRN") public {
    ...
    function setTaxFee(uint256 _taxFee) external onlyOwner {
        require(_taxFee > 0 && _taxFee <= 40, "NIRON: Tax Fee out of range!");
        taxFee = _taxFee;
    }
    
    function setRewardDivider(uint256 _rewardDivider) external onlyOwner {
        require(_rewardDivider >= 0 && _rewardDivider <= 90, "NIRON: RewardDivider out of range!");
    ...
    function swapAndDistribute() private lockTheSwap {
        ...
        uint256 amountToReward = (address(this).balance).mul(rewardDivider).div(100);
        uint256 amountToPool = (address(this).balance).sub(amountToReward);
        
        totalEthDistributed = totalEthDistributed.add(amountToPool);
        IStaking(stakingPool).distribute{value: amountToPool}();
    }
    
		
     //Withdrawing stuck BNBs from the contract in case people send BNBs there^^
	function withdrawStuckBNBs() public onlyOwner {	
