...
contract ERC1155Tradable is ERC1155, ERC1155MintBurn, ERC1155Metadata, Ownable, MinterRole, WhitelistAdminRole {
	...
	// collection address => nft id = this NFTs id
	// required to determine if creation or minting needs to be performed
	mapping(address => mapping ( bytes => uint256 ) ) public converted;
	mapping(bytes => ConvertedNft) public convertedMap;
	...
	function isConverted(address _erc721Address, bytes calldata _id) external view returns(bool){
	    return converted[_erc721Address][_id] > 0;
	}
	
	function getConvertedId(address _erc721Address, bytes calldata _id) external view returns(uint256){
	    return converted[_erc721Address][_id];
	}
	
	function getConvertedSrc(uint256 _id) external view returns(address,uint256){
	    bytes memory _b_id = abi.encodePacked(_id);
	    return (convertedMap[_b_id]._address,convertedMap[_b_id]._i_id);
	}
	
	function resetConversion(address _erc721Address, bytes calldata _id) external onlyWhitelistAdmin{
	    
	    converted[_erc721Address][_id] = 0;
	}
	...
	function create(
	    ...
	    require(_amount > 0, "Amount mustn't be zero.");
	    bytes memory srcIdBytes = abi.encodePacked(_srcId);
	    require(converted[_srcAddress][srcIdBytes] == 0, "Converted already.");
		...
		converted[_srcAddress][srcIdBytes] = _id;
		ConvertedNft memory _converted = ConvertedNft({
	        _address : _srcAddress,
	        _i_id : _srcId
	    });
	    convertedMap[abi.encodePacked(_id)] = _converted;
	    tokenSupply[_id] = _amount;
		tokenMaxSupply[_id] = _amount;
		return _id;
	}
	...
	function burn(address _address, uint256 _id, uint256 _amount) external onlyWhitelistAdmin {
 ...
 */
contract Uniftyverse721 is ERC1155Tradable {
    
    string private _contractURI = "https://unifty.io/meta/uniftyverse721.json";
    
	constructor(address _proxyRegistryAddress) public ERC1155Tradable("Uniftyverse 721", "UNIF721", _proxyRegistryAddress) {
