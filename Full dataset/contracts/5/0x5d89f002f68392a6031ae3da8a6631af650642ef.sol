...
contract LilXToken is Context, IERC20, Ownable {
    ...
    uint256 private _tRewardsPoolTotal;

    string private _name = "LilX | https://t.me/lilXchat";
    ...
    uint256 public _rewardspoolFee = 10;
    uint256 private _previousRewardsPoolFee = _rewardspoolFee;

    uint256 public _maxTxAmount = 1000000 * 10**6 * 10**9;
    uint256 private minimumTokensBeforeSwap = 100 * 10**6 * 10**9;

    address payable public rewardspoolAddress = 0x00392153d513988147C2786b085d249ff91E7E91; // BuyBack Wallet
    ...
    function totalRewardsPoolETH() public view returns (uint256) {
        // ETH has  18 decimals!
        return _tRewardsPoolTotal;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _tRewardsPoolTotal = _tRewardsPoolTotal.add(address(this).balance);
        TransferRewardsPoolETH(rewardspoolAddress, address(this).balance);
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_rewardspoolFee).div(
            10**2
        );
    }

    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _rewardspoolFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousRewardsPoolFee = _rewardspoolFee;
    ...
    function restoreAllFee() private {
        ...
        _rewardspoolFee = _previousRewardsPoolFee;
    }
    ...
    function setRewardsPoolFeePercent(uint256 RewardsPoolFee) external onlyOwner() {
        _rewardspoolFee = RewardsPoolFee;
    }
    ...
    function TransferRewardsPoolETH(address payable recipient, uint256 amount) private {
