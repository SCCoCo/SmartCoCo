        ...
        returns (uint256)
    {
        ...
        if (extenStaking > 0){
            ...
            if ((extenStaking.sub(oldStaking)) >= 0) {
                ...
                extenStaking = extenStaking.sub(mStaking.amountOf(account),"overflaw");
            } else {
                extenStaking = 0;
            }
        }
    ...
    function _cancelStaking(address user, uint256 amount) internal {
        ...
        if (newAmount >= amount) {
            ...
            if (extenStaking > 0){
                ...
                if ((extenStaking.sub(oldStaking)) >= 0) {
                    extenStaking = extenStaking.sub(oldStaking, "overflow");
                    extenStaking = extenStaking.sub(mStaking.amountOf(user), "overflow");
                } else {
                    extenStaking = 0;
                }
        ...
        returns (uint256 total)
    {
        ...
        if (extenStaking > 0){
            ...
            if ((extenStaking.sub(oldStaking, "getTotalReward sub overflow")) >= 0) {
                extenStaking = extenStaking.sub(oldStaking, "overflow");
                extenStaking = extenStaking.sub(mStaking.amountOf(_user), "overflow");
            } else {
                extenStaking = 0;
            }
            if (extenStaking > 0) {
    ...
    function claim() public override whenNotPaused returns (bool) {
        ...
        if (iMyExtStaking > 0){
            ...
            if ((iMyExtStaking.sub(iMyOldStaking)) >= 0) {
                iMyExtStaking = iMyExtStaking.sub(iMyOldStaking, "overflow");
                iMyExtStaking = iMyExtStaking.sub(mStaking.amountOf(msg.sender), "overflow");
            } else {
                iMyExtStaking = 0;
            }
        }
