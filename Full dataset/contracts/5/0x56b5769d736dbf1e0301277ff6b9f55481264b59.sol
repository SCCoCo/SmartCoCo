...
contract A5Maker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _a5, address _weth) public {
        factory = _factory;
        a5 = _a5;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            IERC20(token).transfer(factory.getPair(weth, a5), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, a5), new bytes(0));
        return amountOut;
    }

    function _toA5(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, a5));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
