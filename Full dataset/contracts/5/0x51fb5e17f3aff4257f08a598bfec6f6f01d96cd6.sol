...
pragma solidity >=0.5.8 <0.6.8;


contract DRChainERC20  {
	...
	modifier onlyPayloadSize(uint size) {
		require(msg.data.length == size + 4, "msg.length wrong");
		_;
	}
    /**
    ...
    function _transfer(address _from, address _to, uint _value) internal {
        ...
        require(_to != address(0x0), "toAddress is 0");
    ...
    function approve(address _spender, uint256 _value)  public onlyPayloadSize(2*32) returns (bool success) {
        ...
        require(!((_value != 0) && (allowance[msg.sender][_spender] != 0)), "approve not allowable");    
