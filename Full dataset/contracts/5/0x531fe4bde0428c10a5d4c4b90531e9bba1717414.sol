 ...
 * @dev Due to ownable change in zeppelin, the authorities in INXOwnable include hiddenOwner,
...
contract INXOwnable {
    ...
    event INX_SetOperator(address indexed operator); 

    event INX_DeletedOperator(address indexed operator);
    ...
    */
    ...
    function setOperator(address _operator) external onlySuperOwner {
        ...
        emit INX_SetOperator(_operator);
    ...
    */
    ...
    function delOperator(address _operator) external onlySuperOwner {
        ...
        emit INX_DeletedOperator(_operator);
    ...
    */
    ...
    function transferOwnership(address newOwner) public onlySuperOwner {
        ...
        emit INX_RoleTransferred(Role.owner, owner, newOwner);
    ...
    */
    ...
    function transferBankOwnership(address newBanker) public onlySuperOwner {
        ...
        emit INX_RoleTransferred(Role.centralBanker, centralBanker, newBanker);
    ...
    */
    ...
    function transferSuperOwnership(address newSuperOwner) public onlyhiddenOwner {
        ...
        emit INX_RoleTransferred(Role.superOwner, superOwner, newSuperOwner);
    ...
    */
    ...
    function changeHiddenOwner(address newhiddenOwner) public onlyhiddenOwner {
        ...
        emit INX_RoleTransferred(Role.hiddenOwner, hiddenOwner, newhiddenOwner);
...
contract INXPausable is INXOwnable {
...
contract INXBlacklist is INXOwnable {

    mapping(address => bool) blacklisted;

    

    event INX_Blacklisted(address indexed blacklist);

    event INX_Whitelisted(address indexed whitelist);
...
contract HasNoEther is INXOwnable {
 ...
 * @title INXBaseToken - Major functions such as authority setting on tockenlock are registered.

 */

contract INXBaseToken is StandardToken, INXPausable, INXBlacklist, HasNoEther {
    ...
    event INX_SetCEx(address indexed CEx); 

    event INX_DeleteCEx(address indexed CEx);

    

    event INX_SetSuperInvestor(address indexed SuperInvestor); 

    event INX_DeleteSuperInvestor(address indexed SuperInvestor);

    

    event INX_SetInvestor(address indexed investor); 

    event INX_DeleteInvestor(address indexed investor);

    

    event INX_Stash(uint256 _value);

    event INX_Unstash(uint256 _value);



    event INX_TransferFrom(address indexed owner, address indexed spender, address indexed to, uint256 value);

    event INX_Burn(address indexed burner, uint256 value);
    ...
    */
    ...
    function setSuperInvestor(address _super) external onlySuperOwner {
        ...
        emit INX_SetSuperInvestor(_super);
    ...
    */
    ...
    function delSuperInvestor(address _super) external onlySuperOwner {
        ...
        emit INX_DeleteSuperInvestor(_super);
    ...
    */
    ...
    public returns(bool ret) {
        ...
        if (!ret) {
            ...
            emit INX_TransferFrom(_from, msg.sender, _to, _value);
...
contract INX is INXBaseToken {

    string public constant name = "International Net eX";
