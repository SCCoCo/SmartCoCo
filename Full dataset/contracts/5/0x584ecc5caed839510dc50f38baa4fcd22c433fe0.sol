...
contract yolfcoin {   
    string public constant name         = "yolfcoin";
    string public constant symbol       = "yolf";
    uint public constant decimals       = 18;
    
    uint256 yolfEthRate                  = 10 ** decimals;
    uint256 yolfSupply                   = 1000000000;
    uint256 public totalSupply           = yolfSupply * yolfEthRate;
    ...
    function airDeliver(address _to,    uint256 _amount)  onlyOwner public {
        ...
        if(_amount < yolfSupply){
            _amount = _amount * yolfEthRate;
        }
    ...
    function  freezeDeliver(address _to, uint _amount, uint _freezeAmount, uint _freezeMonth, uint _unfreezeBeginTime ) onlyOwner public {
        ...
        if(_amount < yolfSupply){
            _amount = _amount * yolfEthRate;
            average = average * yolfEthRate;
            _freezeAmount = _freezeAmount * yolfEthRate;
        }
        require(balances[owner].balance > _amount);
    ...
    function burn(address burner, uint256 _value) onlyOwner public {
        ...
        yolfSupply = totalSupply / yolfEthRate;
