...
contract TurtleChef is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;  // Last block number that Turtle distribution occurs.
        uint256 accTurtlePerShare; // Accumulated token per share, times 1e12. See below.
        uint256 isFreezone; // LP without Turtle =  1, LP with Turtle = 0
    }

    // TurtleToken
    TurtleToken public turtle;
    ...
    uint256 public diminish = 864000;
    ...
    constructor(
        ...
        burnaddr = _burnaddr;
        turtlePerBlock = 200000000000000000; // 0.2 /Block
        bonusEndBlock = _bonusEndBlock;
        startBlock = _startBlock;
    }

    function changeReward(uint256 _turtlePerBlock) public onlyOwner {
    ...
    // View function to see pending Turtle on frontend.
    function pendingTurtle(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accTurtlePerShare).div(1e12).sub(user.rewardDebt);
    }
    
    // View function to see expected Turtle on frontend.
    function expectedTurtle(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accTurtlePerShare = pool.accTurtlePerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        ...
        return user.amount.mul(accTurtlePerShare).mul(subrate).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 turtleReward = multiplier.mul(turtlePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        turtle.mint(devaddr, turtleReward.div(20));
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(pending > 0) {
            safeTurtleTransfer(msg.sender, pending.mul(subrate));
            safeTurtleTransfer(burnaddr, pending.sub(pending.mul(subrate)));
        }
        if(_amount > 0) {
    ...
    // Safe turtle transfer function, just in case if rounding error causes pool to not have enough turtle.
    function safeTurtleTransfer(address _to, uint256 _amount) internal {
        uint256 turtleBal = turtle.balanceOf(address(this));
        if (_amount > turtleBal) {
            turtle.transfer(_to, turtleBal);
        } else {
            turtle.transfer(_to, _amount);
        }
    }
