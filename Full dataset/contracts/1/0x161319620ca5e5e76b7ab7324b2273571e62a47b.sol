...
contract YFMBVault is Ownable {
    ...
    // Constructor will set the address of YFMB/ETH LP token
    ...
    function pendingRewards(address _staker) public view returns(uint256) {
        ...
        if (block.number > lastRewardBlock && totalStaked != 0) {
            ...
            uint256 YFMBReward = blocksToReward.mul(getRewardPerBlock());
            _accYFMBPerShare = _accYFMBPerShare.add(YFMBReward.mul(1e18).div(totalStaked));
        }
    ...
    function updatePool() public {
        ...
        YFMBToken.transferFrom(rewardPool, address(this), YFMBReward);

        accYFMBPerShare = accYFMBPerShare.add(YFMBReward.mul(1e18).div(totalStaked));
        lastRewardBlock = block.number;

        emit PoolUpdated(blocksToReward, YFMBReward, now);
    }
