...
// File: Lifetime_TaW_Access.sol

contract Lifetime_TaW_Access is ERC721, Ownable {
    uint256 public MAX_PASSES = 1;
    uint256 public RELEASE_TIMESTAMP;
    uint256 public tokenCounter;

    event ogMint(uint256 indexed tokenId);

    constructor (uint256 saleStart, string memory name, string memory symbol) public ERC721 (name, symbol){
        tokenCounter = totalSupply();
        RELEASE_TIMESTAMP = saleStart;
    }

    function createOneOffCollectible(address _mint_address) public returns (uint256) {
        // only mint 1 edition
        require(tokenCounter < MAX_PASSES, "Only one pass available to mint");
        uint256 newItemId = tokenCounter;

        tokenCounter = tokenCounter + 1;
        
        _safeMint(_mint_address, newItemId);
        emit ogMint(newItemId);
    }
