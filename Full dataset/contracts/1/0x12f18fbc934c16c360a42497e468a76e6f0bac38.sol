...
contract WrappedKongz is ERC20("Wrapped Genesis Kongz", "WGK") {
	using SafeMath for uint256;
	...
	function claimNanas() external syncScore(msg.sender, address(0)) syncGlobalScore() {
		...
		uint256 userShare = totalFarmed.mul(holderScores[msg.sender]).div(globalScore);
		uint256 toSend = userShare.sub(holderClaimed[msg.sender]);
		holderClaimed[msg.sender] = userShare;
		...
		BANANA.transfer(msg.sender, toSend);
		emit NanasClaimed(msg.sender, toSend);
	}
