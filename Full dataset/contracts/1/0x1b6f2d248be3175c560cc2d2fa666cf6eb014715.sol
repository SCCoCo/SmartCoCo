...
contract Prime is IERC20 {
    ...
    uint256 private _totalSupply = 540 ether;

    string private _name = "PrimeBurn Token";
    string private _symbol = "PRIME";
    uint8 private _decimals = 18;
    address private __owner;
    bool private limitBuy = true;
    bool private limitHolders = true;

    uint256[] primes = [29, 23, 19, 17, 13, 11, 7, 5];
    ...
    function enableLimitHolders() public onlyOwner {
        limitHolders = true;
    }
    
    function disableLimitHolders() public onlyOwner {
        limitHolders = false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (limitHolders && !shouldIgnore(recipient)) {
            if (isInWhitelist(recipient)) {
                if (_balances[recipient] > 9 ether) {
                    revert();
                }
            } else if (_balances[recipient] > 3 ether) {
                revert();
            }
        }
