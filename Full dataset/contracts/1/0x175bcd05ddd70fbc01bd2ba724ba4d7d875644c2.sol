...
contract Reflexion is ERC20Burnable, Claimable {
    string public constant name    = "Reflexion Token";  
    string public constant symbol  = "XRLO";  
    uint8 public constant decimals = 18; 
    
    uint256 constant initialSupply = 200000000e18;
    ...
    constructor(address _reserveAddress, address _teamAddress, address _advisorsAddress, address _bountyAddress) public {
        reserveAddress = _reserveAddress;
        teamAddress = _teamAddress;
        advisorsAddress = _advisorsAddress;
        bountyAddress = _bountyAddress;

        _mint(msg.sender, 90000000e18); //45%
        _mint(address(this), 98000000e18); //49%
        _mint(bountyAddress, 12000000e18);  //6%
    }
    ...
    function claimReserveTokens() public unlockingPeriodStarted {
        ...
        _transfer(address(this), reserveAddress, 60000000e18); //30%
    }
    
    function claimTeamTokens() public unlockingPeriodStarted {
        require (msg.sender == teamAddress);
        require (now > beginUnlockDate + 133 days);
        require (!teamTokensClaimed);
        teamTokensClaimed = true;
        _transfer(address(this), teamAddress, 32000000e18); //16%
    }
    
    function claimAdvisorsTokens() public unlockingPeriodStarted {
        require (msg.sender == advisorsAddress);
        require (now > beginUnlockDate + 133 days);
        require (!advisorsTokensClaimed);
        advisorsTokensClaimed = true;
        _transfer(address(this), advisorsAddress, 6000000e18); //3%
    }
