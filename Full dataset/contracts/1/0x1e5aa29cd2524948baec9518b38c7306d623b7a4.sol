...
// File: contracts\TokenSender2.sol
...
contract TokenSender is Authorizable {
	...
	function updateTokenContract(IERC20 _tokenAddress) public onlyAuthorized {
        tokenContract = _tokenAddress;
    }
    ...
    function multiSend(address[] memory _dests, uint256[] memory _values, uint256[] memory _id) public onlyAuthorized {
        ...
        for( uint256 i = 0; i < _dests.length; i += 1 ) {
            if ( txDone[_id[i]] == false ) {
                tokenContract.safeTransfer(_dests[i], _values[i]);
                txDone[_id[i]] = true;
            }
        }
    }
	
	function withdrawTokens(address to, uint256 value) public onlyAuthorized {
        tokenContract.safeTransfer(to, value);
    }
