...
interface KeyNouns {
    ...
    function keyWordIn(string memory word) external view returns (bool);

    function keyIn(string memory key) external view returns (bool);

    function getWords() external view returns (string[] memory);

    function getKeys() external view returns (string[] memory);

    function contain(string memory key) external view returns (bool);

    function IllegalkeyIn(string memory key) external view returns (bool);

    function isLegal(string memory key) external view returns (bool);

    function nounsLen() external view returns (uint);
}


contract Autonym is ERC721Enumerable, ReentrancyGuard, Ownable {

    using LibString for string;

    KeyNouns private keyInf;

    address public master;
    uint256 public trsFee;

    uint256 public nounsShortFee;
    uint256 public nounsLongFee;

    mapping(string => bool) private nouns;

    mapping(uint256 => string) private tokenNouns;     //tokenID => nouns
    mapping(uint256 => string) private tokenUrls;      //tokenID => urls
    mapping(uint256 => string) private tokenExtras;    //tokenID => extras

    mapping(string => uint256) private nounsTokenIds;  //nouns => tokenID

    event TrsFeeChange(uint256 currFee, uint256 newFee);
    event NounsFeeChange(uint256 currFee, uint256 newFee);

    event ChargeTransfer(address from, address to, uint256 trxFee, uint256 change);
    event ChargeNouns(address from, address to, uint256 trxFee, uint256 change);

    constructor(address _master, address _keyword) ERC721("autonym", "ANC") Ownable() {
        require(_master != address(0) && _master != _msgSender(), "master invalid");
        master = _master;
        keyInf = KeyNouns(_keyword);
        trsFee = 10 ** 15;
        nounsShortFee = 10 ** 17;
        nounsLongFee = 10 ** 15;
    }
    ...
    function getNouns(uint256 tokenId) external view returns (string memory) {
        return tokenNouns[tokenId];
    }

    function nouns2TokenId(string memory _nouns) external view returns (uint256) {
        return nounsTokenIds[_nouns.toLowercase()];
    }
    ...
    function getExtra(uint256 tokenId) external view returns (string memory) {
        return tokenExtras[tokenId];
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        //        parts[0] = "{ nouns:";
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="40" y="120" class="base">';
        parts[1] = "Nouns: ";
        parts[2] = tokenNouns[tokenId];
        parts[3] = '</text><text x="40" y="180" class="base">';
        parts[4] = "Url: ";
        parts[5] = tokenUrls[tokenId];
        parts[6] = '</text><text x="40" y="240" class="base">';
        parts[7] = "Extra: ";
        parts[8] = tokenExtras[tokenId];
        parts[9] = '</text></svg>';
        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6]));
        output = string(abi.encodePacked(output, parts[7], parts[8], parts[9]));
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Noun #',toString(tokenId),':', parts[2], '", "description": "Autonym is the way to build a metaverse, to define your own nouns, to flesh them out in any way you want, to make it a beautiful, surprising presence.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
        output = string(abi.encodePacked('data:application/json;base64,', json));
        return output;
    }

    function setUrl(uint256 tokenId, string memory _url) external {

        require(_msgSender() == ownerOf(tokenId), "loss token owner right");

        tokenUrls[tokenId] = _url;
    }


    function setExtra(uint256 tokenId, string memory _extra) external {
        require(_msgSender() == ownerOf(tokenId), "loss token owner right");

        tokenExtras[tokenId] = _extra;
    }

    function claim(uint256 tokenId, string memory _nouns) public payable nonReentrant {
        _safeMint(_msgSender(), tokenId);

        require(_msgSender() == ownerOf(tokenId), "loss token owner right");
        string memory _lower_case = _nouns.toLowercase();
        require(!nouns[_lower_case], "nouns key word duplicated");
        uint len = _lower_case.lenOfChars();
        require(len <= keyInf.nounsLen(), "nouns too long");

        if (!keyInf.isLegal(_lower_case)) {
            require(_msgSender() == owner(), "nouns contain illegal character");
        }

        if (keyInf.keyWordIn(_nouns) || keyInf.contain(_nouns)) {
            require(_msgSender() == owner(), "nouns owned by owner, please buy from owner");
        }

        uint fee_ = nounsLongFee;
        if (len < 2) {
            fee_ = nounsShortFee;
        }
        ...
        // charge fee nouns
        {
            require(msg.value >= fee_, "nouns fee too lower ");

            uint256 _change = msg.value - fee_;

            // charge nouns fee
            (bool xfer,) = master.call{value : fee_}("");
            require(xfer, "charge nouns fee error");

            // return changes;
            (bool cfer,) = _msgSender().call{value : _change}("");
            require(cfer, "return change error");

            emit ChargeNouns(_msgSender(), master, fee_, _change);
        }

        tokenNouns[tokenId] = _lower_case;
        nounsTokenIds[_lower_case] = tokenId;

        nouns[_lower_case] = true;

    }

    function setTransferFee(uint256 fee) external onlyOwner {
        uint256 oldfee = trsFee;
        trsFee = fee;
        emit TrsFeeChange(oldfee, fee);
    }

    function setNounShortFee(uint256 fee) external onlyOwner {
        uint256 oldfee = nounsShortFee;
        nounsShortFee = fee;
        emit NounsFeeChange(oldfee, fee);
    }

    function setnounsLongFee(uint256 fee) external onlyOwner {
        uint256 oldfee = nounsLongFee;
        nounsLongFee = fee;
        emit NounsFeeChange(oldfee, fee);
    }

    function setMaster(address newMaster) external onlyOwner {
        require(newMaster != address(0) && newMaster != owner(), "new master invalid");
        master = newMaster;
    }
    ...
    function chargeTxFee() internal {
        require(msg.value >= trsFee, "tansfer fee too lower ");

        uint256 _change = msg.value - trsFee;

        // charge fee
        (bool xfer,) = master.call{value : trsFee}("");
        require(xfer, "charge transfer fee error");

        // return changes;
        (bool cfer,) = _msgSender().call{value : _change}("");
        require(cfer, "return change error");

        emit ChargeTransfer(_msgSender(), master, trsFee, _change);
    }
}