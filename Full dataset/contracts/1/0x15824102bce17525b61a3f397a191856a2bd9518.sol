...
contract MyV2FlashLoan is FlashLoanReceiverBase {
    ...
    address public warpControlAddress =
        0x8E0Fa7c5C7Fa86A059e865A90b50a90351df716a;
    ...
    address public susdAddress = 0x57Ab1ec28D129707052df4dF418D58a2D46d5f51;
    ...
    function executeOperation(
        ...
        // liquidate account
        (vars.borrower, vars.warpControlAddress, vars.warpTeam) = abi.decode(
            params,
        ...
        // 2. convert Uniswap WETH-sUSD to USDC
        vars.token0 = wethAddress;
        vars.token1 = susdAddress;
        convertToUSDC(vars);
    ...
    function convertUSDCToToken(opVars memory vars) private {
        ...
        vars.router.swapTokensForExactTokens(
            vars.amount,
            1e64,
            getPath(usdcAddress, vars.token0),
            address(this),
            1e18
        );
    }
    ...
    function convertToUSDC(opVars memory vars) private {
        ...
        // convert eth to USDC
        vars.router.swapExactETHForTokens.value(amountETH)(
            0,
            getPath(vars.router.WETH(), usdcAddress),
            address(this),
            1e18
        );
        ...
        if (vars.token1 != usdcAddress) {
            ...
            vars.router.swapExactTokensForTokens(
                amountToken,
                0,
                getPath(vars.token1, usdcAddress),
                address(this),
                1e18
            );
        }
