...
contract SOLARITERebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 solaritesSold, uint256 solaritesFromReserves, uint256 solaritesToReserves);
    ...
    /// @notice pair for reserveToken <> SOLARITE
    ...
    /// @notice Whether or not this token is first in uniswap SOLARITE<>Reserve pair
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(solariteAddress_, reserveToken_);

          // used for interacting with uniswap
          if (token0 == solariteAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap SOLARITE<>Reserve pair
    ...
    */
        ...
        public
    {
        ...
        SOLARITETokenInterface solarite = SOLARITETokenInterface(solariteAddress);

        if (positive) {
            require(solarite.solaritesScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < solarite.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = solarite.totalSupply();
        ...
        assert(solarite.solaritesScalingFactor() <= solarite.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            solarite.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.solaritesToUni) {
                // if the amount from reserves > solaritesToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                solarite.mint(uniswap_pair, uniVars.solaritesToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            solarite.mint(uniswap_pair, uniVars.solaritesToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            solarite.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.solaritesToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
        ...
        internal
    {
        ...
        uint256 excess = solarite.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          solaritesToUni: tokens_to_max_slippage, // how many solarites uniswap needs
          amountFromReserves: excess, // how much of solaritesToUni comes from reserves
          mintToReserves: 0 // how much solarites protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.solaritesToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire solarite reserves, and minting part of total
