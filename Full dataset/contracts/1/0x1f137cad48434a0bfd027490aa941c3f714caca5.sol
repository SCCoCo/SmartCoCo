...
def woof(amount: uint256 = MAX_UINT256, receiver: address = msg.sender) -> bool:
    ...
    assert ERC20(SHIB).transferFrom(msg.sender, self, mint_amount*(10**10)*100000)
    assert ERC20(AKITA).transferFrom(msg.sender, self, mint_amount*(10**10)*1000000)
...
def unwoof(amount: uint256 = MAX_UINT256, receiver: address = msg.sender) -> bool:
    ...
    assert ERC20(SHIB).transfer(receiver, burn_amount*(10**10)*100000)
    assert ERC20(AKITA).transfer(receiver, burn_amount*(10**10)*1000000)
    assert ERC20(WDOGE).transfer(receiver, burn_amount*(10**10))
    assert ERC20(renDOGE).transfer(receiver, burn_amount)	
    assert ERC20(WOOFY).transfer(receiver, burn_amount*(10**4)*10)		
    return True
