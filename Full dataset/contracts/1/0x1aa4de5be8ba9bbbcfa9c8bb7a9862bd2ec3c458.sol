...
contract TokenStakable is ERC20
{
    ...
    mapping(address/*asset*/ => mapping(uint32/*refCode*/ => uint256/*price*/)) public assets; // address(0) = ETH
    ...
    event SetPrice(address indexed asset, uint32 indexed refCode, uint256 price);
    ...
    constructor(address _owner,
                ...
                uint32[] memory _assetRefCodes,
                uint256[] memory _assetPrices) public ERC20(_owner, _name, _symbol)
    {
        require(_assetAddresses.length == _assetRefCodes.length
                && _assetAddresses.length == _assetPrices.length);
        ...
        {
            assets[_assetAddresses[i]][_assetRefCodes[i]] = _assetPrices[i];
            
            emit SetPrice(_assetAddresses[i], _assetRefCodes[i], _assetPrices[i]);
        }
    ...
    function setPrices(address[] calldata _assetAddresses,
                       uint32[] calldata _assetRefCodes,
                       uint256[] calldata _assetPrices) external onlyOwner
    {
        require(_assetAddresses.length == _assetRefCodes.length
                && _assetAddresses.length == _assetPrices.length,
                "ERR_ARRAYS_LENGTHS_DONT_MATCH");
    ...
    // Price calculator:
    //--------------------------------------------------------------------------------------------------------------------------
    function calcPrice(address _fromAsset, uint256 _fromAmount, uint32 _refCode) public view returns (uint256 toActualAmount_, uint256 fromActualAmount_)
    {
        require(_fromAmount > 0, "ERR_ZERO_PAYMENT");
        
        uint256 fromAssetPrice = assets[_fromAsset][_refCode];
    ...
    function _swap(address _fromAsset, uint256 _fromAmount, uint256 _toExpectedAmount, uint32 _refCode) private
    {
        ...
        (uint256 toActualAmount, uint256 fromActualAmount) = calcPrice(_fromAsset, _fromAmount, _refCode);
    ...
    {
        return _a > _b ? (_a - _b <= 10**15) : (_b - _a <= 10**15);
    }
    //--------------------------------------------------------------------------------------------------------------------------
