...
contract BlockchipToken is StandardToken, Owned {

    string public constant name = "BlockChip";
    ...
    /// Maximum tokens to be allocated (880 million BCHIP)

    uint256 public constant HARD_CAP = 888000000 * 10**uint256(decimals);
    ...
    constructor(address _teamTokensAddress, address _reserveTokensAddress, address _advisorsTokensAddress,

                address _saleTokensAddress, address _bountyTokensAddress, address _airdropTokensAddress) public {
        ...
        whitelisted[airdropTokensAddress] = true;

        whitelisted[bountyTokensAddress] = true;

        whitelisted[advisorsTokensAddress] = true;
        ...
        uint256 saleTokens = 417360000 * 10**uint256(decimals);
        ...
        /// Reserve tokens - 266 400 000 BCHIP (30%)

        uint256 reserveTokens = 266400000 * 10**uint256(decimals);
        ...
        /// Advisors tokens - 26 640 000 BCHIP (3%)

        uint256 advisorsTokens = 26640000 * 10**uint256(decimals);
        ...
        /// Airdrop tokens - 26 640 000 BCHIP (3%)

        uint256 airdropTokens = 26640000 * 10**uint256(decimals);

        totalSupply_ = totalSupply_.add(airdropTokens);

        balances[airdropTokensAddress] = airdropTokens;

        emit Transfer(address(0), airdropTokensAddress, airdropTokens);



        /// Bonty and referral tokens - 44 400 000 BCHIP (5%)

        uint256 bountyTokens = 44400000 * 10**uint256(decimals);
        ...
        /// Team tokens - 106 560 000 BCHIP (12%)

        uint256 teamTokens = 106560000 * 10**uint256(decimals);

        totalSupply_ = totalSupply_.add(teamTokens);

        teamTokensLock = new TokenTimelock(this, teamTokensAddress, uint64(now + 547 days));
