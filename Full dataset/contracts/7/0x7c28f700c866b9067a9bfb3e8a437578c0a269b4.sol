...
contract QubAirdrop20200924 is Ownable {
    using SafeERC20 for IERC20;

    uint256 public CLAIM_AMOUNT = 1813784764207981*10**8;
    ...
    uint256 public addressesCountInWhitelist = 0;
    ...
    function claim() public payable checkWhitelist checkPayment checkClaim {
    ...
    function addToWhitelist(address[] memory addresses) public onlyOwner {
        for (uint i = 0; i < addresses.length; i++) {
            if (!inWhitelist[addresses[i]]) {
                inWhitelist[addresses[i]] = true;
                addressesCountInWhitelist++;
            }
        }
    }
    
    function removeFromWhitelist(address participant) public onlyOwner {
        inWhitelist[participant] = false;
        addressesCountInWhitelist--;
    }
