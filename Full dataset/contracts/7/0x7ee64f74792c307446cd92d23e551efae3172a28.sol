...
// File contracts/MigratableWethTransmuter.sol
 ...
 */
contract MigratableWethTransmuter is Context {
    using SafeMath for uint256;
    ...
    constructor(address _waToken, address _weth, address _governance) public {
        require(_governance != ZERO_ADDRESS, "MigratableWethTransmuter: 0 gov");
    ...
    modifier onlyWhitelisted() {
        require(whiteList[msg.sender], "MigratableWethTransmuter: !whitelisted");
        _;
    }
    ...
    ///
    modifier onlyGov() {
        require(msg.sender == governance, "MigratableWethTransmuter: !governance");
        _;
    }
    ...
    function claim() public {
        ...
        IWETH(weth).transfer(sender, value);
    }
    ...
    function unstake(uint256 amount) public updateAccount(msg.sender) {
        ...
        address sender = msg.sender;
        require(depositedWaTokens[sender] >= amount,"MigratableWethTransmuter: unstake amount exceeds deposited amount");
        ...
        updateAccount(toTransmute)
    {
        ...
        require(
            ...
            "MigratableWethTransmuter: !overflow"
        );
        ...
        if (realisedTokens[toTransmute] > 0) {
            ...
            IWETH(weth).transfer(toTransmute, value);
        }
    }
    ...
    function distribute(address origin, uint256 amount) public onlyWhitelisted() runPhasedDistribution() {
        require(!pause, "emergency pause enabled");
        IWETH(weth).transferFrom(origin, address(this), amount);
    ...
    function setPendingGovernance(address _pendingGovernance) external onlyGov {
        require(_pendingGovernance != ZERO_ADDRESS, "MigratableWethTransmuter: 0 gov");
