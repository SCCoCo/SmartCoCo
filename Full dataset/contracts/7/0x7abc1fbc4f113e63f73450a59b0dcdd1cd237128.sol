...
contract Gartier is Authorizable, TokenERC20 {
    ...
    /// Base exchange rate is set to 1 ETH = GTR.
   ...
   /// no tokens can be ever issued when this is set to "true"
    ...
    constructor(
        ...
        tokenSaleHardCap = 50550550550 * 10**uint256(decimals); // Default Crowsale Hard Cap amount with decimals
        baseRate = 5000000 * 10**uint256(decimals); // Default base rate GTR) :1 eth amount with decimals
    }
   ...
   /* function _transfer(address _from, address _to, uint _value) override internal {
    ...
    function mintToken(address target, uint256 mintedAmount) public onlyAuthorized {
    ...
    function freezeAccount(address target, bool freeze) public onlyAuthorized {
    ...
    function setRatePrices(uint256 newRate) public onlyAuthorized{
        baseRate = newRate;
    }
    ...
    /// @dev Do issues tokens to target beneficiary
