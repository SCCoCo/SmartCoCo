...
contract ICARUS is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Icarus Token";
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if(_cooldownEnabled) {
                    if(buyLimitEnd > block.timestamp) {
                        ...
                        trader[to].buyCD = block.timestamp + (10 seconds);
                    }
                }
            }
            ...
            if(!inSwap && from != uniswapV2Pair && tradingOpen) {
                ...
                if(contractTokenBalance > 0) {
                    if(contractTokenBalance > balanceOf(uniswapV2Pair).mul(8).div(100)) {
                        contractTokenBalance = balanceOf(uniswapV2Pair).mul(8).div(100);
                    }
    ...
    function openTrading() external onlyOwner() {
        ...
        tradingOpen = true;
        buyLimitEnd = block.timestamp + (5 minutes);
    ...
    function setNotTaxMode(bool onoff) external {
        require(_msgSender() == _FeeAddress);
        _noTaxMode = onoff;
    }
