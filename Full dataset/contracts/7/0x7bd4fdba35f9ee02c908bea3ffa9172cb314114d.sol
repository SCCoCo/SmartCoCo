...
contract MambaCoin is ERC20 {
    bytes4 private constant SELECTOR = bytes4(keccak256(bytes('initShareholder(address)')));

    constructor(address redenvelopAddr) ERC20("Mamba Coin", "MAMB") {
        uint256 initialSupply = 19780823.20200126 ether;

        uint256 toTeam = initialSupply*(24+8)/100;
        _mint(msg.sender, toTeam);
        _mint(redenvelopAddr, initialSupply - toTeam);

        // init Mamb to RedEnvelop
