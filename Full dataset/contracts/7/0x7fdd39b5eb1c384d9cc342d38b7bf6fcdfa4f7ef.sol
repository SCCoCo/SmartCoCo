...
// File: contracts/CraneMasterFarmer.sol
...
interface IMigratorToCraneSwap {
    // Perform LP token migration from legacy UniswapV2 to Crane.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // CraneSwap must mint EXACTLY the same amount of Crane LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// CraneMasterFarmer is the master ofCrane. He can makeCrane and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Crane is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract CraneMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount ofCrane
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accCranePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accCranePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool.Crane to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Cranes distribution occurs.
        uint256 accCranePerShare; // Accumulated Crane per share, times 1e12. See below.
    }
    ...
    // Crane tokens created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus muliplier for early Crane makers.
    ...
    // The block number when Crane mining starts.
    ...
    IMigratorToCraneSwap public migrator;
    ...
    event SendCraneReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "CraneMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's CRANE allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorToCraneSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 craneForDev;
        uint256 craneForFarmer;
        (craneForDev, craneForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        
        if (craneForDev > 0) {
            crane.mint(devaddr, craneForDev);
            //For more simple, I lock reward for dev if mint reward in bonus time
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                crane.lock(devaddr, craneForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            }
        }
        crane.mint(address(this), craneForFarmer);
        pool.accCranePerShare = pool.accCranePerShare.add(craneForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forFarmer) {
        ...
        uint256 craneCanMint = crane.cap().sub(crane.totalSupply());

        if (craneCanMint < amount) {
            forDev = 0;
            forFarmer = craneCanMint;
        }
        else {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 craneForFarmer;
            (, craneForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accCranePerShare = accCranePerShare.add(craneForFarmer.mul(1e12).div(lpSupply));
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            uint256 masterBal = crane.balanceOf(address(this));
            ...
            if(pending > 0) {
                crane.transfer(msg.sender, pending);
                uint256 lockAmount = 0;
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    lockAmount = pending.mul(PERCENT_LOCK_BONUS_REWARD).div(100);
                    crane.lock(msg.sender, lockAmount);
                }

                user.rewardDebtAtBlock = block.number;

                emit SendCraneReward(msg.sender, _pid, pending, lockAmount);
            }
    ...
    // Deposit LP tokens to CraneMasterFarmer for Crane allocation.
    ...
    // Withdraw LP tokens from CraneMasterFarmer.
