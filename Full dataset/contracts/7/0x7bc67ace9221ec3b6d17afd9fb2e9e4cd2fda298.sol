...
contract Resolver is Helpers {
    ...
    function getSinglePosition(address user, address pool, address stakingPool) public view returns(UserData memory _data) {
        _data.pool = pool;
        StakingInterface stakingContract = StakingInterface(stakingPool);
    ...
    function getPosition(address user, address[] memory pools, address[] memory stakingPool) public view returns(UserData[] memory _data) {
        _data = new UserData[](pools.length);
        for (uint i = 0; i < pools.length; i++) {
            _data[i] = getSinglePosition(user, pools[i], stakingPool[i]);
        }
    }
    ...
    */
    function getWithdrawParams(address user, address pool, address stakingPool, uint burnPercent, uint slippage) public view returns (uint burnAmt, uint amount0, uint amount1, uint amount0Min, uint amount1Min) {
        UserData memory _data = getSinglePosition(user, pool, stakingPool);
...
contract InstaGUNIV3PoolResolver is Resolver {
    string public constant name = "G-UNI-V3-Resolver-v1.1";
}