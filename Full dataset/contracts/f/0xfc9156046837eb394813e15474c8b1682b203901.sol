...
contract PantherToken is ERC20("PantherSwap Token", "PANTHER"), Ownable {
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by a minter.
    ...
    */
     ...
     */
    modifier onlyMinter() {
        require(_minters.contains(_msgSender()), "PANTHER: caller is not a minter");
        _;
    }

    /// @notice View minters length
    function mintersLength() public view returns (uint256) {
        return _minters.length();
    }

    /// @notice Check address is a minter
    ...
    function addMinter(address _minter) public onlyOwner {
        require(_minter != address(0), "PANTHER: zero address given");
        require(!_minters.contains(_minter), "PANTHER: minter already exists");

        _minters.add(_minter);
        emit MinterAdded(msg.sender, _minter);
    }

    /// @notice Remove a minter
    function removeMinter(address _minter) public onlyOwner {
        require(_minter != address(0), "PANTHER: zero address given");
        require(_minters.contains(_minter), "PANTHER: the minter does not exist");
