...
// ClbrMaker is MasterChef's left hand and kinda a wizard. He can cook up Clbr from pretty much anything!
//
// This contract handles "serving up" rewards for xClbr holders by trading tokens collected from fees for Clbr.
...
contract ClbrMaker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _clbr, address _weth) public {
        factory = _factory;
        clbr = _clbr;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, clbr), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, clbr), new bytes(0));
        return amountOut;
    }

    // Converts WETH to Clbr
    function _toCLBR(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, clbr));
