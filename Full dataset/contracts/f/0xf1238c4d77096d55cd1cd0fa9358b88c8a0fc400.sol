...
contract GLSC is UtilGLSC, WhitelistAdminRole {
    ...
    function persAddMoney() public isHuman() payable {
        ...
        User storage user = userRoundMapping[rid][msg.sender];
        investAmout = user.freezeAmount.add(msg.value);
        require(user.id != 0, "user not exist");
        user.lineAmount = lineAmount;
        require(investAmout >= 1 * ethWei && investAmout <= 10 * ethWei, "between 1 and 10");
