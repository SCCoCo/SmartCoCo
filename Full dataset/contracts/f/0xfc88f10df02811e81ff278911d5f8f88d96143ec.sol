...
contract MyAdvancedToken is owned, TokenERC20 {
    ...
    /// @notice Transfer initial coins to `target` and prevent from sending & receiving tokens
    /// @param target Address to be frozen and send token
    /// @param _value amount of coins
    function freezeAccountAndAddToken(address target, uint256 _value) onlyOwner public {
        frozenAccount[target] = true;
        FrozenFunds(target, true);

        _transfer(msg.sender, target, _value);
    }
