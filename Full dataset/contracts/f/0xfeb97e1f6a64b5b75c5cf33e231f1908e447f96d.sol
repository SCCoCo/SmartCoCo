...
contract Strategy is CurveVoterProxy {
    ...
    address public constant eurt = address(0xC581b735A1688071A1746c968e0798D642EDE491);

    constructor(address _vault) public CurveVoterProxy(_vault) {
        curve = address(0xFD5dB7463a3aB53fD211b4af195c5BCCC1A03890);
        gauge = address(0xe8060Ad8971450E624d5289A10017dD30F5dA85F);
        keepCRV = 1000;
    ...
    function _approveBasic() internal override {
        IERC20(eurt).safeApprove(curve, 0);
        IERC20(eurt).safeApprove(curve, type(uint256).max);
    }

    // >>> approve other rewards on dex
    function _approveDex() internal override {
        super._approveDex();
        IERC20(usdt).safeApprove(uniswapv3, 0);
        IERC20(usdt).safeApprove(uniswapv3, type(uint256).max);
    }
        ...
        )
    {
        ...
        uint256 _eurt = IERC20(eurt).balanceOf(address(this));
        if (_eurt > 0) {
            ICurveFi(curve).add_liquidity([_eurt, 0], 0);
        }
