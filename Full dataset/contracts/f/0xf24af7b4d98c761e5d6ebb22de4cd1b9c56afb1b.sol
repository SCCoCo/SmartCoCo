...
contract JPOPDOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _approve(
        ...
        require(false, "JPOPDOGE_Dividend_Tracker: No approvals allowed");
    }
    ...
    function _transfer(
        ...
        require(false, "JPOPDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(
            false,
            "JPOPDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main JPOPDOGE contract."
        );
    }
...
contract JPOPDOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    JPOPDOGEDividendTracker public dividendTracker;

    address payable public devAddress =
        0x6E106C8f3618D5abC69660B4d86A912c699Ad35b;
    ...
    constructor() public ERC20("JPOPDOGE", "JPOPDOGE") {
        IFTPAntiBot _antiBot = IFTPAntiBot(
            0x590C2B20f7920A2D21eD32A21B616906b4209A43
        );
        antiBot = _antiBot;

        dividendTracker = new JPOPDOGEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "JPOPDOGE: The dividend tracker already has that address"
        );

        JPOPDOGEDividendTracker newDividendTracker = JPOPDOGEDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "JPOPDOGE: The new dividend tracker must be owned by the JPOPDOGE token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "JPOPDOGE: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "JPOPDOGE: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "JPOPDOGE: The UniSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "JPOPDOGE: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function setMaxPurchaseEnabled(bool enabled) public onlyOwner {
        require(
            ...
            "JPOPDOGE: Max purchase enabled is already the value of 'enabled'"
        );
        maxPurchaseEnabled = enabled;
    }
    ...
    function reinvestInactive(address payable account) public onlyOwner {
        ...
        require(
            ...
            "JPOPDOGE: Account balance must be less then minimum token balance for dividends"
        );

        uint256 _lastTransfer = lastTransfer[account];
        require(
            block.timestamp.sub(_lastTransfer) > 12 weeks,
            "JPOPDOGE: Account must have been inactive for at least 12 weeks"
        );
    ...
    function _claim(
        ...
        require(
            ...
            "JPOPDOGE: Claimer has no withdrawable dividend"
        );
    ...
    function _transfer(
        ...
        if (tradingStartTime > block.timestamp) {
            require(
                ...
                "JPOPDOGE: This account cannot send tokens until trading is enabled"
            );
        }
        ...
        if (maxPurchaseEnabled && automatedMarketMakerPairs[from]) {
            require(
                ...
                "JPOPDOGE: Exceeds max purchase amount"
            );
        }
