...
//Dynamic % Burn Ratee (Deflation)//

Changelog:
- Removed role based permissions
- Replaced fixed burn rate with dynamic burn rate
- Added max burnrate (Just to be sure that the burnrate cant be higher => 30%)
- Added fee exception part (Exception to transfer fees, for example for Uniswap contracts)
- Mint initial supply on contract deployment
- Mint function (Owner of (this) token contract needs to be set to (bancore farm) the farming contract.
- Adjusted the emission rate.
(After changing ownership the contract creator can't call any functions assigned to OnlyOwner).
...
contract BanCORE is ERC20, Ownable {
  constructor() public ERC20("BANCORE.PRO", "BANv2") { // Symbol and Name
    // Mint initial 9000 BANv2 Tokens
  ...
  uint256 public transferFee; // Fee in percentage, where 123 = 1.23%
  ...
  function setTransferFee(uint256 fee) public onlyOwner {
    require(fee <= 3000, "Fee cannot be greater than 30%");
    if (fee == 0) {
      activeFee = false;
    } else {
      activeFee = true;
  ...
  // TransferFrom recipient receives amount, sender's account is debited amount + fee
