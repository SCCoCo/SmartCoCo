...
contract SGRTokenManager is ISGRTokenManager, ContractAddressLocatorHolder {
    ...
    /**
    * @dev Return the contract which implements the ISGRToken interface.
    */
    function getSGRToken() public view returns (SGRToken) {
        return SGRToken(getContractAddress(_ISGRToken_));
    }
     ...
     */
    function exchangeEthForSgr(address _sender, uint256 _ethAmount) external returns (uint256) {
        require(false, "minting new SGR is disabled");
        _sender;
        _ethAmount;
        return 0;
    }
     ...
     * @return The amount of ETH that the sender is entitled to.
     */
    function exchangeSgrForEth(address _sender, uint256 _sgrAmount) external only(_ISGRToken_) returns (uint256) {
        require(_sender != address(0), "sender is illegal");
        SGRToken sgrToken = getSGRToken();
        
        require(_sgrAmount <= sgrToken.balanceOf(_sender), "unauthorized SGR amount to exchange");
        
        uint256 ethBalance = sgrToken.getEthBalance();
        uint256 sgrTokenTotalSupply = sgrToken.totalSupply();

        uint256 ethAmount = (_sgrAmount.mul(ethBalance)).div(sgrTokenTotalSupply);
    ...
    */
     ...
     * @param _balance The amount of ETH in the SGRToken contract.
     * @param _amount The deposited ETH amount.
     * @return Fixed zero as not in use.
     */
     ...
     */
    function uponMintSgrForSgnHolders(uint256 _value) external {
        require(false, "minting SGR for SGN holders is disabled");
        _value;
    }
     ...
     */
    function uponTransferSgrToSgnHolder(address _to, uint256 _value) external {
        require(false, "transferring SGR to SGN holders is disabled");
        _to;
        _value;
    }
     ...
     */
    function postTransferEthToSgrHolder(address _to, uint256 _value, bool _status) external {
        require(false, "postTransferEthToSgrHolder is disabled");
        _to;
        _value;
        _status;
    }
