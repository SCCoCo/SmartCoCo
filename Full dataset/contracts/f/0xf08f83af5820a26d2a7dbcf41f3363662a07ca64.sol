...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(base).length == 0) {
            return "https://lootcodebucket.s3.us-east-2.amazonaws.com/baseimage.png";
        }
...
/**
 * @title LootCode contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract LootCode is ERC721, ReentrancyGuard, Ownable {
    using SafeMath for uint256;

    string public PROVENANCE = "";
    uint256 public lootersPrice = 0; //0.00 ETH
    uint256 public publicPrice = 20000000000000000; //0.025 ETH
    ...
    constructor() ERC721("LootCode (for world)", "LootCode (for world)") {
    }
    ...
    function mintWithLoot(uint lootId) public payable nonReentrant {
        ...
        require(totalSupply()  < 2000, "Private sale minting limit exceeds");
    ...
    function multiMintWithLoot(uint[] memory lootIds) public payable nonReentrant {
        require(saleIsActive, "Sale must be active to mint");
        require(totalSupply() + lootIds.length < 2001, "Private sale minting limit exceeds");
    ...
    function mint(uint lootId) public payable nonReentrant {
        ...
        require(lootId > 0 && lootId < 10001, "Token ID invalid");
        require(!_exists(lootId), "This token has already been minted");
    ...
    function multiMint(uint[] memory lootIds) public payable nonReentrant {
        ...
        for (uint i=0; i<lootIds.length; i++) {
            require(lootIds[i] > 0 && lootIds[i] < 10001, "Token ID invalid");
