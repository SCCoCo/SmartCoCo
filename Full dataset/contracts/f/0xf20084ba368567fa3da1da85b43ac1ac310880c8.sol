...
// File: contracts/bAlphaToken.sol
...
// File: contracts/bAlphaMaster.sol
...
contract bAlphaMaster is Ownable {
    using SafeMath for uint256;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "bAlphaMaster::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        bAlpha.mint(address(this), reward);
    ...
    function safeTransferReward(address _to, uint256 _amount) internal {
        uint256 bal = bAlpha.balanceOf(address(this));
        if (_amount > bal) {
            bAlpha.transfer(_to, bal);
        } else {
            bAlpha.transfer(_to, _amount);
        }
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint) {
        ...
        uint256 amountCanMint = bAlpha.cap().sub(bAlpha.totalSupply());
