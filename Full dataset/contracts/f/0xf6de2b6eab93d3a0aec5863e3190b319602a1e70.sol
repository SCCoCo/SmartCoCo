...
// File: contracts/LiquidtyStaking.sol
...
// This is a Liquidty Token Mining Contract that will allow users to deposit or "stake" Liquidty Pool Tokens to 
// earn rewards in the form of the FMTA Token. It is designed to be highly configureable so it can adapt to market
// and ecosystem conditions overtime. Liqudidty pools must be added by the contract owner and only added Tokens
// will be eleigible for rewards.  The contract also uses Role Based Access Control (RBAC) to allow other accounts 
// and contracts (such as oracles) to function as `_ADMIN` allowing them to securly interact with the contract 
// providing possible future extensibility.
...
// have a choice of three lock periods to choose from all with different DPY and CDPY factors.

// For example lets use 7, 14 and 21 days as our choices for lock periods and have our user Stake 1000 LP Tokens: 
...
contract LiquidityMining is Ownable, AccessControl {
    ...
    uint256 private compYield0;
    uint256 private compYield1;
    uint256 private compYield2;
    
    uint256 private lockPeriodBPScale;
    uint256 public maxUserBP;
    
    uint256 private preYieldDivisor;
    ...
    constructor() {
        ...
        lockPeriod0BasisPoint = 3200;
        lockPeriod1BasisPoint = 3800;
        lockPeriod2BasisPoint = 4500;
        preYieldDivisor = 2;
        maxUserBP = 5500;
        compYield0 = 50;
        compYield1 = 75;
    ...
    function setLockPeriodBasisPoints (
        uint256 _newLockPeriod0BasisPoint, 
        uint256 _newLockPeriod1BasisPoint, 
        uint256 _newLockPeriod2BasisPoint) public {
        require(hasRole(_ADMIN, msg.sender),"LiquidityMining: Message Sender must be _ADMIN");
    ...
    function setLockPeriodBPScale(uint256 _newLockPeriodScale) public {
        require(hasRole(_ADMIN, msg.sender),"LiquidityMining: Message Sender must be _ADMIN");
        lockPeriodBPScale = _newLockPeriodScale;
    
    }

    function setMaxUserBP(uint256 _newMaxUserBP) public {
        require(hasRole(_ADMIN, msg.sender),"LiquidityMining: Message Sender must be _ADMIN");
    ...
    function setCompoundYield (
        uint256 _newCompoundYield0, 
        uint256 _newCompoundYield1, 
        uint256 _newCompoundYield2) public {
        require(hasRole(_ADMIN, msg.sender),"LiquidityMining: Message Sender must be _ADMIN");
    ...
    function forcePositionRemoval(uint256 _pid, address _account) public {
    ...
    function calculateUserDailyYield(uint256 _pid) public view returns (uint256 _dailyYield) {
        ...
        PoolInfo memory pool = poolInfo[_pid];
        uint256 dailyYield = p.LockedAmount.mul(p.UserBP.add(pool.PoolBonus)).div(lockPeriodBPScale).mul(p.Days);
        return dailyYield;
    }
    ...
    function moveERC20(address _ERC20, address _dest, uint256 _ERC20Amount) public {
        require(hasRole(_MOVE, msg.sender));
        IERC20(_ERC20).safeTransfer(_dest, _ERC20Amount);

    }

    function ethRescue(address payable _dest, uint256 _etherAmount) public {
