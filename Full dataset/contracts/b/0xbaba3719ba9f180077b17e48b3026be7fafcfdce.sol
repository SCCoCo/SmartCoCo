...
// Root file: contracts/MateriaLiquidityRemover.sol
...
contract MateriaLiquidityRemover is MateriaOperator {
    ...
    function removeLiquidity(
        ...
        address to,
        uint deadline
    ) public ensure(deadline) returns (uint amountBridge, uint amountToken) {
        ...
        {
        (bool ethItem, uint itemId) = _isEthItem(token, erc20Wrapper);
        ...
        (uint amount0, uint amount1) = IMateriaPair(pair).burn(to);
    ...
    function removeLiquidityETH(
        ...
        address to,
        uint deadline
    ) public ensure(deadline) returns(uint amountBridge, uint amountEth) {
        address erc20Wrapper = address(IMateriaOrchestrator(address(this)).erc20Wrapper());
        address bridgeToken = address(IMateriaOrchestrator(address(this)).bridgeToken());
        ...
        IMateriaPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IMateriaPair(pair).burn(address(this));
        ...
        _unwrapEth(uint(IMateriaOrchestrator(address(this)).ETHEREUM_OBJECT_ID()), amountEth, erc20Wrapper, to);
    }
