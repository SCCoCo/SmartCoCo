 ...
 */
contract TopDogBeachClub is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    /// @dev Solidity 0.8.0+ gives us safe math ops by default so there is no need to use SafeMath == cheaper gas

    enum FoundMemberClaimStatus { Invalid, Unclaimed, Claimed }
    ...
    mapping (address => FoundMemberClaimStatus) private _foundingMemberClaims;
    ...
    */
    ...
    function mintFoundingMember() external payable nonReentrant() {
        ...
        require(_foundingMemberClaims[msg.sender] != FoundMemberClaimStatus.Claimed, "You've already claimed your doggo");
        require(_foundingMemberClaims[msg.sender] == FoundMemberClaimStatus.Unclaimed, "You are not a founding member");
        ...
        _foundingMemberClaims[msg.sender] = FoundMemberClaimStatus.Claimed;
    }

    function addFoundingMembers(address[] memory members) external onlyOwner {
        for (uint256 i = 0; i < members.length; i++) {
            _foundingMemberClaims[members[i]] = FoundMemberClaimStatus.Unclaimed;
        }
    }
