    ...
    Ownable
{
    ...
    function unlock(uint256 _tokenId) public{
        require(_balances[_tokenId][msg.sender] == 1,"you have to own this token");
    ...
    function _getUri(uint256 _tokenId) internal view returns (string memory) {
        if(unlocked[_tokenId]){
            return string(abi.encodePacked(URI_BASE, Strings.toString(_random(_tokenId)),".txt"));
        }
        else{
    ...
    function setRandstr(string memory _str)
        external
        onlyOwner
    {
        randstr = _str;
    ...
    function _random(uint256 a) private view returns (uint) {
        return (uint(keccak256(abi.encodePacked(a,randstr))));
    }
    
    function setSalesStart(bool _salesStart)
        external
        onlyOwner
    {
