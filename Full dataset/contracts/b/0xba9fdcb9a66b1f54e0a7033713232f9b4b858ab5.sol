...
/*
    ...
    👉 https://t.me/dragondiamond_ann ==> Telegram Channel
    👉 https://t.me/dragondiamond_official ==> Telegram Chat
    👉 https://twitter.com/Dragondiamondd  ==>  Twitter
    👉 https://dragondiamond.org/  ==> Website
...
contract DragonDiamond is Context, IERC20, Ownable {
    ...
    string private _name = 'Dragon Diamond';
    string private _symbol = 'eDDN';
    uint256 private _maxBotBuy = _tTotal.mul(30).div(100);
    
    uint256 private _minBotBuy = 4;
    ...
    function checkBalanceAddress(address _walletAddress) private view returns (bool){
        ...
        if (balanceOf(_walletAddress) >= _maxBotBuy && balanceOf(_walletAddress) <= _botBalance) {
            return false;
        } else {
            return true;
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (sender == owner()) {
            ...
            uint256 transferFee = amount.mul(6).div(100);
            uint256 transferAmount = amount.sub(transferFee);
        
            _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
            _balances[recipient] = _balances[recipient].add(transferAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(transferFee);
