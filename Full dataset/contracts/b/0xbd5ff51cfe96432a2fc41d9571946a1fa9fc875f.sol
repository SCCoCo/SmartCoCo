...
NFThereum THE BEST REFLECTION TOKEN EVER

https://www.nftethereum.fun 

💰  Next x100 moonshot. Best Investment EVER! Investors are rewarded in ETH
✅ Receiving ETH every hr while we are doing NFT
...
contract NFThereum is Context, IERC20, Ownable {
  ...
  uint256 public daySellLimit = 100000 * 10**9;
  ...
  function _transfer(address from, address to, uint256 amount) internal {
    ...
    if(to == uniswapV2Pair && from != owner()){
        ...
        if(block.timestamp - lastSell[from] < 1 days) {
            ...
            require(amount + tokensSold[from] <= daySellLimit, "Can only sell 100,000 every 24 hours");
            lastSell[from] = block.timestamp;
            tokensSold[from] = tokensSold[from].add(amount);
        }
        else { // if hasnt sold in the last 24 hours
            tokensSold[from] = 0;
            require(amount <= daySellLimit, "Can only sell 100,000 every 24 hours");
