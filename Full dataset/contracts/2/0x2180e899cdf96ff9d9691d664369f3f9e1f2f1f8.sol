...
contract IdolToken is ERC20 {
    ...
    constructor(address _minter) ERC20("IDOL", "IDOL") {
        ...
        minter = _minter;
        owner = _msgSender();
        //I will use this inital supply of 10000 IDOL and 1 ether to create a uniswap pool
        _mint(_msgSender(), 10000 * 10**18);
    }

    function mintDaysLeft() public view returns (uint256) {
        if (block.timestamp - deployTime < 1000 days) {
            return 1000 - (block.timestamp - deployTime) / 86400;
        } else {
            return 0;
        }
    }
    ...
    function _canMintTo(address _address) private view returns (bool) {
        if (_mintTimeMap[_address].length < 10) {
            return true;
        } else {
            if (block.timestamp - _mintTimeMap[_address][_mintTimeMap[_address].length - 10] < 1 weeks) {
                return false;
            } else {
                return true;
            }
    ...
    function mintForCreateNFT(address _to, uint256 _amountInWei) external {
        require(_msgSender() == minter, "IdolToken: mint request from unauthorized address");
    ...
    function mintForBuyNFT(address _to, uint256 _priceInWei) external {
        require(_msgSender() == minter, "IdolToken: mint request from unauthorized address");
        if (mintDaysLeft() > 0 && _canMintTo(_to)) {
            uint256 _mintAmount = mintDaysLeft() * _priceInWei;
            _mint(_to, _mintAmount);
            //mint additional 10% to dapp creator
            if (_to != owner) {
                _mint(owner, _mintAmount / 10);
