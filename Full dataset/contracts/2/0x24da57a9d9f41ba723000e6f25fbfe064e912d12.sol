...
library SyscoinMessageLibrary {
    ...
    // @dev 每 Read a bytes32 from an offset in the byte array
    ...
    // @dev 每 Read an uint32 from an offset in the byte array
...
contract SyscoinBattleManager is SyscoinErrorCodes {
    ...
    // @dev 每 Configures the contract managing superblocks battles
...
contract SyscoinClaimManager is SyscoinDepositsManager, SyscoinErrorCodes {
    ...
    // @dev 每 runs a battle session to verify a superblock for the next challenger
    // @param superblockHash 每 claim id.
    ...
    // @dev 每 Return session by challenger
