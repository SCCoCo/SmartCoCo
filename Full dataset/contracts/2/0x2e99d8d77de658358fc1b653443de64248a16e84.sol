 ...
 * elliptic curve primitives and hashes. Calling this internally from another function will lead to 
    ...
    **/
    ...
    function() external payable {
        ...
        assembly {
            ...
            // We don't check for function signatures, there's only one function that ever gets called: 

            // validateJoinSplit()

            // We still assume calldata is offset by 4 bytes so that we can represent this contract through
            ...
            * We can recover \bar{k}_{n-1} from the homomorphic sum condition 

            * \sum_{i=0}^{m-1}\bar{k}_i = \sum_{i=m}^{n-1}\bar{k}_i + k_{public}

            * So we use the empty slot to store k_{public}, which represents any public 'value' being blinded

            * into zero-knowledge notes

            *

            * We use a hard-coded memory map to reduce gas costs - if this is not called as an external contract then 
            ...
            * 0x300:???       = block of memory that contains 

            *                       (\gamma_i, \sigma_i)_{i=0}^{n-1} concatenated with (B_i)_{i=0}^{n-1}
            ...
            **/
            ...
            function validateJoinSplit() {
                ...
                for { let i := 0 } lt(i, n) { i := add(i, 0x01) } {
                    ...
                    // As per the homomorphic sum condition: 

                    // \sum_{i=0}^{m-1}\bar{k}_i = \sum_{i=m}^{n-1}\bar{k}_i + k_{public}c, 
                ...
                // a JoinSplit transaction. We can inductively assume that all input notes are well-formed 

                // AZTEC commitments and do not need to validate the implicit range proof
            ...
            **/
            ...
            function validatePairing(t2) {
                ...
                // indices are a bit off, scipr lab's libff limb ordering (c0, c1) is opposite to what 

                // precompile expects

                // We can overwrite the memory we used previously as this function is called at the end of 
            ...
            * and that signatures 'k' and 'a' are modulo the order of the curve. Transaction will throw if 
                        ...
                        and(
                            ...
                            eq( // y^2 ?= x^3 + 3
                                ...
                                addmod(
                                    ...
                                    mulmod(mulmod(sigmaX, sigmaX, field_order), sigmaX, field_order), 3, field_order
                            ...
                            eq( // y^2 ?= x^3 + 3
                                ...
                                addmod(
                                    ...
                                    mulmod(mulmod(gammaX, gammaX, field_order), gammaX, field_order), 3, field_order
            ...
            * This is used both as an input to validate the challenge `c` and also to generate 

            * pseudorandom relationships between commitments for different outputNotes,

            * so that we can combine them into a single multi-exponentiation for the purposes of
