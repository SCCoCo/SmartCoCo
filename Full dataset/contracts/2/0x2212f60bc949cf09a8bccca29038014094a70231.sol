...
contract zsToken is ERC20("Stabilize Strategy DAI Token", "zs-DAI"), Ownable {
    ...
    constructor (IERC20 _asset) public {
    ...
    function valueOfVaultAndStrategy() public view returns (uint256) { // The total value of the tokens
        ...
        if(currentStrategy != StabilizeStrategy(address(0))){
            ...
            balance += currentStrategy.balance().mul(currentStrategy.pricePerToken()).div(1e18); // Get the rep token and its worth in underlying token    
        }    
        return balance;
    }
    ...
    function redeem(uint256 share) public {
        ...
        if(currentStrategy != StabilizeStrategy(address(0))){
            ...
            if(share < tokenTotal){
                uint256 _balance = _underlyingAsset.balanceOf(address(this)); // The token
