...
// Users by default withdraw from ust pool before collateral pool
...
contract zsToken is ERC20("Stabilize Strategy UST", "zs-UST"), Ownable, ReentrancyGuard {
    ...
    // Token information
    // This is a single asset wrapper that only accepts UST
    // UST, USDT
    struct TokenInfo {
    ...
    function withdrawTokenReserves() public view returns (address, uint256) {
        // This function will return the address and amount of the token of ust, and if none available, the collateral asset
        if(currentStrategy != StabilizeStrategy(address(0))){
    ...
    function redeem(uint256 share) public nonReentrant {
        ...
        // Users cannot choose which token they get. They get UST then USDT if available
    ...
    // This will withdraw the tokens from the contract based on order, essentially ust then collateral
