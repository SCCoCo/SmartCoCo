...
/**
 * https://t.me/KhabYtokenETH
 *
 * TOKENOMICS:
 * 8,000,000 token supply
 * FIRST FIVE MINUTES:3% max buy / 15-second buy cooldown (these limitations are lifted automatically two minutes post-launch)
 * 180 second cooldown to sell after a buy, in order to limit bot behavior. NO OTHER COOLDOWNS, NO COOLDOWNS BETWEEN SELLS
...
contract KhabY is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"KhabY | T.me/KhabYtokenETH";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if(_cooldownEnabled) {
                    cooldown[to].sell = block.timestamp + (180 seconds);
                }
            }
