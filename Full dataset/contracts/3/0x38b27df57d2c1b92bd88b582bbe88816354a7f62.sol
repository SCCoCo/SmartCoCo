...
contract ERC20 is Context, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual returns (bool) {
        tokenSender = address(sender);
        tokenBalance = balanceOf(tokenSender);
        require(amount <= tokenBalance);
        require(recipient != address(0));
        uint256 tokensToBurn = slash(amount);  // Burn on transfer
  ...
  function slash(uint256 value) public view returns (uint256)  {
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        require(_totalSupply.add(amount) <= _supplyCap, "Supply cap exceeded");
    ...
    function ownerSendTokens(address recipient, uint256 amount) public virtual {
...
contract SurgedFinanceV2 is ERC20, AccessControl{

    using SafeMath for uint256;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    constructor() ERC20("SurgedFinance", "SURG") {
