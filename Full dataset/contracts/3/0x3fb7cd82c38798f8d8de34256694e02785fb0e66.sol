...
contract UniAPE is Context, IERC20, Ownable {
    ...
    string private _name = 'UniAPE';
    string private _symbol = 'uAPE';
    uint8 private _decimals = 9;

    uint256 private _taxFee = 10; // 5% reflection fee for every holder
    uint256 private _burnFee = 0; // burn fee
    uint256 private _liquidityFee = 5; // 5% into liquidity

    uint256 private _previousTaxFee = _taxFee;
    uint256 private _previousburnFee = _burnFee;
    uint256 private _previousLiquidityFee = _liquidityFee;

    address payable private _burnWalletAddress = payable(0x0000000000000000000000000000000000000001);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        sendETHToburn(fromSwap.sub(newBalance));
    }
    ...
    function sendETHToburn(uint256 amount) private {
        _burnWalletAddress.transfer(amount);
    }
    ...
    function manualSend() public onlyOwner() {
        ...
        sendETHToburn(contractETHBalance);
    }
    ...
    function _setburnFee(uint256 burnFee) external onlyOwner() {
        require(burnFee >= 0 && burnFee <= 49, 'burnFee should be in 0 - 11');
        _burnFee = burnFee;
    }
