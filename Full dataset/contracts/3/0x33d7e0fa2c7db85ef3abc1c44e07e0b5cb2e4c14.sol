...
contract Strategy is ConvexStable {
    ...
    constructor(address _vault) public ConvexStable(_vault) {
        ...
        _setPathTarget(2, 0); // dfd path target
    ...
    function _approveDex() internal override {
        super._approveDex();
        IERC20(dfd).approve(dex[2], 0);
        IERC20(dfd).approve(dex[2], type(uint256).max);
    }

    // >>> include other rewards
    function _migrateRewards(address _newStrategy) internal override {
        super._migrateRewards(_newStrategy);
        IERC20(dfd).safeTransfer(_newStrategy, IERC20(dfd).balanceOf(address(this)));
    }
        ...
        )
    {
        ...
        if (_dfd > 0) {
            ...
            Uni(dex[2]).swapExactTokensForTokens(_dfd, uint256(0), path, address(this), now);
        }
