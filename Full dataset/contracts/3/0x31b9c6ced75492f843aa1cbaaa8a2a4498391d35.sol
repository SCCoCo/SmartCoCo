...
contract MyV2FlashLoan is FlashLoanReceiverBase {
        ...
        returns (bool)
    {
        ...
        (vars.borrower, vars.warpControlAddress, vars.warpTeam, vars.assetVaults) 
          = abi.decode(params, (address, address, address, address[]));
       
        for (uint i = 0; i < assets.length; i++) {
            // approve asset withdraw by flash loan provider
            IERC20(assets[i]).safeApprove(vars.assetVaults[i], uint256(-1));
        }
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            IERC20(assets[i]).safeApprove(address(LENDING_POOL), uint256(-1));
    ...
    function liquidateWarpAccount(
        ...
        bytes memory params = abi.encode(borrower, warpControl, warpTeam, assetVault);
    ...
    function convertUSDCToToken(opVars memory vars) private
    {
        ...
        IERC20 usdcToken = IERC20(usdcAddress);
        usdcToken.approve(address(vars.router), uint256(-1));
    ...
    function convertToUSDC(opVars memory vars) private
    {
        ...
        if(vars.token1 != usdcAddress)
        {
          ...
          daiToken.approve(address(vars.router), uint256(-1));
