...
firstReward: public(HashMap[uint256, uint256])
...
def mint(
    ...
    _firstReward: uint256,
) -> bool:
    assert _firstReward != 0
    assert _tokenURI != ''
    assert msg.sender == self.minter
    assert _to != ZERO_ADDRESS
    self._addTokenTo(_to, _tokenId)
    self.tokenURIs[_tokenId] = _tokenURI
    self.firstReward[_tokenId] = _firstReward
    self.totalSupply += 1
...
@external
def sell(_tokenId: uint256, _price: uint256):
...
@external
def stopSelling(_tokenId: uint256):
    assert self._isApprovedOrOwner(msg.sender, _tokenId)
    owner: address = self.idToOwner[_tokenId]
    assert owner != ZERO_ADDRESS
