...
contract UniswapV2Recipe is Ownable {
    ...
    function toPie(address _pie, uint256 _poolAmount) external payable {
        ...
        for(uint256 i = 0; i < tokens.length; i++) {
            ...
            (uint256 reserveA, uint256 reserveB) = UniswapV2Library.getReserves(address(uniswapFactory), address(WETH), tokens[i]);
            uint256 amountIn = UniswapV2Library.getAmountIn(amounts[i], reserveA, reserveB);
    ...
    function calcToPie(address _pie, uint256 _poolAmount) public view returns(uint256) {
        ...
        for(uint256 i = 0; i < tokens.length; i++) {
            (uint256 reserveA, uint256 reserveB) = UniswapV2Library.getReserves(address(uniswapFactory), address(WETH), tokens[i]);
            totalEth += UniswapV2Library.getAmountIn(amounts[i], reserveA, reserveB);
        }
    ...
    function toEth(address _pie, uint256 _poolAmount, uint256 _minEthAmount) external {
        ...
        for(uint256 i = 0; i < tokens.length; i++) {
            (uint256 reserveA, uint256 reserveB) = UniswapV2Library.getReserves(address(uniswapFactory), tokens[i], address(WETH));
            uint256 wethAmountOut = UniswapV2Library.getAmountOut(amounts[i], reserveA, reserveB);
            IUniswapV2Exchange pair = IUniswapV2Exchange(uniswapFactory.getPair(tokens[i], address(WETH)));
    ...
    function calcToEth(address _pie, uint256 _poolAmountOut) external view returns(uint256) {
        ...
        for(uint256 i = 0; i < tokens.length; i++) {
            (uint256 reserveA, uint256 reserveB) = UniswapV2Library.getReserves(address(uniswapFactory), tokens[i], address(WETH));
            totalEth += UniswapV2Library.getAmountOut(amounts[i], reserveA, reserveB);
        }
