...
contract RTK_SwapContract {
    ...
    */
    ...
    function getMinimumRequiredAMMOAmount() public view OwnerOnly returns (uint256) {
        uint256 netUnburntExtCirculation = 0;
        for(uint i = 1; i <= 4; i++) {
            netUnburntExtCirculation += (_RTKLX_ExtCirculation[i] - ERC20(_Token_RTKL[i]).balanceOf(address(0)))*(i);
        }
    ...
    function getMinimumRequiredRTKAmount() public view OwnerOnly returns (uint256) {
        uint256 netUnburntExtCirculation = 0;
        for(uint i = 1; i <= 4; i++) {
            netUnburntExtCirculation += (_RTKLX_ExtCirculation[i] - ERC20(_Token_RTKL[i]).balanceOf(address(0)));
        }
