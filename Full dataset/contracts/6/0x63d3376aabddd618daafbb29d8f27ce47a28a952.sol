...
// File: contracts/vaults/yearnv2/v032/strategies/GenericLevComp.sol
 ...
 *
 ********************* */
/// @notice Fork of Yearns Leveraged comp farming strategy
...
contract GenericLevComp is BaseStrategy, DydxFlashloanBase, ICallee {
    ...
    function setForceMigrate(bool _force) external onlyOwner {
        forceMigrate = _force;
    }
    ...
    //emergency function that we can use to deleverage manually if something is broken
    function manualReleaseWant(uint256 amount) external onlyOwner {
        require(cToken.redeemUnderlying(amount) ==0, "failed redeem");
    }
