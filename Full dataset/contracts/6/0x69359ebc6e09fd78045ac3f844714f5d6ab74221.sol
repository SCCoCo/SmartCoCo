...
Fair Launch 
No Private / Public Sale

Passive Income in ETH
/*
...
contract FlokiETH is ERC20, Ownable {
    ...
    FlokiETHDividendTracker public dividendTracker;
    
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;

    uint256 public MAX_BUY_TRANSACTION_AMOUNT = 500000000 * (10**18); 
    uint256 public MAX_SELL_TRANSACTION_AMOUNT = 30000000 * (10**18); 
    uint256 public MAX_WALLET_AMOUNT = 10000000000 * (10**18);
    ...
    function activateTrading() public onlyOwner {
        require(!tradingEnabled, "FlokiETH: Trading is already enabled");
        tradingEnabled = true;
    }
    ...
    constructor() ERC20("FlokiETH", "FLOETH") {
        assert(TOTAL_FEES == 15);

        dividendTracker = new FlokiETHDividendTracker();
        ...
        */
        _mint(owner(), 10000000000 * (10**18)); // 10 Billion (100%)
    }

    receive() external payable {}

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FlokiETH: The dividend tracker already has that address");

        FlokiETHDividendTracker newDividendTracker = FlokiETHDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FlokiETH: The new dividend tracker must be owned by the FlokiETH token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FlokiETH: The router already has that address");
        emit UpdatedUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account) public onlyOwner {
        require(!_isExcludedFromFees[account], "FlokiETH: Account is already excluded from fees");
        _isExcludedFromFees[account] = true;
    }
    
    function includeForFees(address account) public onlyOwner {
        require(_isExcludedFromFees[account], "FlokiETH: Account is already included for fees");
    ...
    function setSellFeeIncreaseFactor(uint256 multiplier) external onlyOwner {
  	    require(SELL_FEE_INCREASE_FACTOR >= 100 && SELL_FEE_INCREASE_FACTOR <= 200, "FlokiETH: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    SELL_FEE_INCREASE_FACTOR = multiplier;
  	}

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FlokiETH: The Uniswap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FlokiETH: Automated market maker pair is already set to that value");
    ...
    function allowTransferBeforeTradingIsEnabled(address account) public onlyOwner {
        require(!canTransferBeforeTradingIsEnabled[account], "FlokiETH: Account is already allowed to transfer before trading is enabled");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        ...
        require(newValue != gasForProcessing, "FlokiETH: Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    function updateLiquidationThreshold(uint256 newValue) external onlyOwner {
        require(newValue != liquidateTokensAtAmount, "FlokiETH: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "FlokiETH: This account cannot send tokens until trading is enabled");
        }
...
contract FlokiETHDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FlokiETH_Dividend_Tracker", "FlokiETH_Dividend_Tracker") {
        claimWait = 3600;
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "FlokiETH_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "FlokiETH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FlokiETH contract.");
    }
    ...
    function updateGasForTransfer(uint256 newGasForTransfer) external onlyOwner {
        require(newGasForTransfer != gasForTransfer, "FlokiETH_Dividend_Tracker: Cannot update gasForTransfer to same value");
        emit GasForTransferUpdated(newGasForTransfer, gasForTransfer);
        gasForTransfer = newGasForTransfer;
    }

    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FlokiETH_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FlokiETH_Dividend_Tracker: Cannot update claimWait to same value");
