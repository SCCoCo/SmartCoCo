...
contract Collectible is ERC721, AccessControl{
    ...
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Collectible: Caller is not an Admin");
        _;
    }
    
    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), "Collectible: Caller is not a Minter");
        _;
    }

    function grandAccountToMinter(address _account) public onlyAdmin{
        grantRole(MINTER_ROLE, _account);
    }

    function revokeAccountFromMinter(address _account) public onlyAdmin{
    ...
    function mintTo(address _to, bytes32 _hash) public onlyMinter{
        uint256 _currentTokenId = currentTokenId;
        URIHashesMapping[_currentTokenId] = _hash;
        _mint(_to, _currentTokenId);
        _currentTokenId++;
        currentTokenId = _currentTokenId;
    }

    function getURIHash(uint256 _tokenId) public view returns(string memory){  
        require(_exists(_tokenId), "Collectible: URI query for nonexistent token");
        return IPFSlib.encode(URIHashesMapping[_tokenId]);
    }
    ...
    */
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        require(_exists(_tokenId), "Collectible: URI query for nonexistent token");
        return IPFSlib.concatStrings(baseTokenURI(),getURIHash(_tokenId));
    }
