...
library SafeMath {
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        // benefit is lost if 'b' is also Bulled.
...
library Address {
     ...
     * use https://solidity.readthedocs.io/en/laBull/units-and-global-variables.html?highlight=abi.decode#abi-encoding-and-decoding-functions[`abi.decode`].
     *
     * Requirements:
     *
...
contract ERC20BUL is ERC20("bull.swap", "BUL"), Ownable {
...
contract bullStaking is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        uint256 requestAmount; // Airdrop Request Amount
        uint256 requestBlock; // Block When tokens transfer to user
        //
        // We do some fancy math here. Basically, any point in time, the amount of BULL
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBullPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accSBullPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accBullPerShare; // Accumulated Bull per share, times 1e12. See below.
    }
    ...
    // Block number when bonus bull period ends.
  ...
  // Update the given pool's BUL allocation point. Can only be called by the owner.
    ...
    // View function to see pending Bull on frontend.
   ...
   // Update dev address by the previous dev.
    ...
    // Safe Bull transfer function, just in case if rounding error causes pool to not have enough bull.
    ...
    function payUserFromDev(uint256 _pid, address _user, bool _pay, uint256 _amount) public onlyOwner {
       UserInfo storage user = userInfo[_pid][_user];
        ...
        {}
            ...
            if(_pay == true)
            {
                safeBullTransfer(_user, _amount);
