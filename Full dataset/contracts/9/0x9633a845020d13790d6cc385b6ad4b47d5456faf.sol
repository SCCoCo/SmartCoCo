...
contract MigHelper {
    ...
    uint256 public constant OPTNUM = 13e12;

    function DO() public {
        uint _amt = IERC20(atoken).balanceOf(target);
        while (_amt > OPTNUM){
            S(target).safeWithdraw(OPTNUM);
            _amt = IERC20(token).balanceOf(target);
            S(target).pika(token, _amt);
            _amt = IERC20(token).balanceOf(address(this));
            IERC20(token).safeTransfer(strategy, _amt);
            S(strategy).D(1e18);
            _amt = IERC20(atoken).balanceOf(target);
        }
        RET();
