...
contract Forwarder {
    ...
    constructor(address payable destination) public {
    ...
    function flushTokenQuantity(address tokenContractAddress, uint256 quantity) public {
        ERC20Interface instance = ERC20Interface(tokenContractAddress);
        if (!instance.transfer(destinationAddress, quantity)) {
            revert();
        }
