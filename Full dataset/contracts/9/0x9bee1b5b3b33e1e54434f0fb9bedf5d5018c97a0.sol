...
contract LYX {
	...
	uint256 constant private Total_LYX = 10e22;
	uint256 constant private Burn_Ratio = 25; 
	uint256 constant private Burn_Threshold = 5;     
	uint256 constant private Freeze_Size = 1e20; 
	
	string constant public name = "Lukso";
	string constant public symbol = "LYX";
	...
	constructor() public {
		...
		info.totalSupply = Total_LYX;
		info.users[msg.sender].balance = Total_LYX;
		emit Transfer(address(0x0), msg.sender, Total_LYX);
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		if (totalSupply() - _burnedAmount < Total_LYX * Burn_Threshold / 100 || isWhitelisted(_from)) {
			_burnedAmount = 0;
		}
