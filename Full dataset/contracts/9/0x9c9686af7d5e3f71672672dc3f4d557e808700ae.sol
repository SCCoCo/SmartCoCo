    ...
    private
  {
    ...
    if (address(fromToken) == address(data.aToken)) {
      if (address(toToken) == Utils.ethAddress()) {
        IWETHGateway(lData.wethGateway).withdrawETH(fromAmount, address(this));
      }
      else {
      ...
      if (address(fromToken) == Utils.ethAddress()) {
        IWETHGateway(lData.wethGateway).depositETH{value : fromAmount}(address(this), lData.refCode);
      }
      else {
