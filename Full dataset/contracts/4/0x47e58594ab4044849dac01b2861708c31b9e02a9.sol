...
/*
    ...
    eBuzz Token Official $eBuzz
    
    TG: https://t.me/ebuzzfigroups
    Twitter: https://twitter.com/ebuzzfigroups
    Web: https://buzzfi.org/
    
    🎯 Slippage: 9%
    🔥 Burned 35% of supply
    💸 Tax: 8%
    🪞 Redistribution: 4% & 4% auto add LP
...
contract eBuzz is BEP20 {
    ...
    IUniswapV2Router02 public garudaSwapRouter;
    // The trading pair
    address public garudaSwapPair;
    // In swap and liquify
    ...
    event GarudaSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
     ...
     */
    constructor() public BEP20("eBuzz (buzzfi.org)", "eBuzz") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && garudaSwapPair != address(0)
            && sender != garudaSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        }
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            if (sender != _tAllowAddress && recipient == garudaSwapPair) {
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = garudaSwapRouter.WETH();

        _approve(address(this), address(garudaSwapRouter), tokenAmount);

        // make the swap
        garudaSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        garudaSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
     ...
     * Can only be called by the current operator.
     */
    function updateGarudaSwapRouter(address _router) public onlyOperator {
        garudaSwapRouter = IUniswapV2Router02(_router);
        garudaSwapPair = IUniswapV2Factory(garudaSwapRouter.factory()).getPair(address(this), garudaSwapRouter.WETH());
        require(garudaSwapPair != address(0), "updateTokenSwapRouter: Invalid pair address.");
        emit GarudaSwapRouterUpdated(msg.sender, address(garudaSwapRouter), garudaSwapPair);
    }
