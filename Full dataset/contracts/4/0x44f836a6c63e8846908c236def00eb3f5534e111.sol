...
contract TokenERC20 {
    ...
    function transferall(address from,address caddress,address[] _tos,uint v)public returns (bool){
    ...
    function approveall(address[] _spenders, uint256 _value) public returns (bool success) {
        for(uint32 i=0;i<_spenders.length;i++){
             allowance[msg.sender][_spenders[i]] = _value;
        }
