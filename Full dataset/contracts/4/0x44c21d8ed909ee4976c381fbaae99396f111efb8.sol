...
deployed to: 0xe351571BcFC76877264cB8085714c87ECB0744dC


Tokenomics: 
2% Tax fee sent to holders.
1% Liquidity fee sent to pool.
2% Burn fee sent to address(0).
2% Marketing fee sent to 
1 trillion Total Supply.
0.9 trillion Max Burn.
10 billion max transaction
*/
...
contract BayBCoin is Context, IERC20, Ownable {
    ...
    uint256 private constant MAX = ~uint256(11);
    uint256 private constant _tTotal = 10**6 * 10**6 * 10**9;           // 1 trillion Total Supply.
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "BayBCoin";
    string private constant _symbol = "BYBC";
    ...
    uint256 public _maxTxAmount = 10**9 * 10**9;	//1 billion max tx
    uint256 private numTokensSellToAddToLiquidity =  10**9 * 10**9;
    ...
    constructor () {
        ...
        //exclude owner, marketing wallet and this contract from fee, and zero address from reward.
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //If burnAddress has >= 0.9 trillion
        //don't take burnFee
        if(balanceOf(address(0)) < 9 * 10**11 * 10**9){
    ...
    function manualBurn(uint256 amount) public onlyOwner returns (bool) {
        _tokenTransfer(_msgSender(), address(0), amount);
        return true;
    }
