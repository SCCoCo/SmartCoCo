...
/**
 *    __ _   _                ___
 *   /__\ |_| |__   ___ _ __ / _ \___   ___ _ __ ___  ___
 *  /_\ | __| '_ \ / _ \ '__/ /_)/ _ \ / _ \ '_ ` _ \/ __|
 * //__ | |_| | | |  __/ | / ___/ (_) |  __/ | | | | \__ \
 * \__/  \__|_| |_|\___|_| \/    \___/ \___|_| |_| |_|___/
 *
 */
...
contract Etherpoems is ERC721, Ownable {
  ...
  mapping(uint256 => uint256) public rarefractalsNFTClaimed;

  uint256 public constant MAX_NFT_SUPPLY = 206;
  uint256 public constant currentPrice = 100000000000000000;
  address internal rarefractals;
  ...
  function mint(
    ...
    _validateTokenToBeMinted(_myTokenURI, _poemName);
    ...
    // Mint the token
    _mint(_myTokenURI, _poemName, _poemText, _poemAuthor);
  }

  /**
   * Community grant minting for each Rarefractals NFT Owner.
   */
  function mintWithRareFractals(
    uint256 _rareFractalsTokenID,
    string memory _myTokenURI,
    string memory _poemName,
    string memory _poemText,
    string memory _poemAuthor
    ...
    // RareFractals NFT is with in valid range
    require(
      _rareFractalsTokenID >= 0 && _rareFractalsTokenID < 500,
      "Invalid RareFractals NFT"
    );
    // RareFractals NFT is not already used to claim etherPoem NFT
    require(
      rarefractalsNFTClaimed[_rareFractalsTokenID] == 0,
      "Already minted with this RareFractals NFT"
    );
    // etherPoem claimer owns the RareFractals NFT
    require(
      IERC721(rarefractals).ownerOf(_rareFractalsTokenID) == msg.sender,
      "Not the owner of this RareFractals NFT"
    );
    _validateTokenToBeMinted(_myTokenURI, _poemName);
    // Mark the RareFractals NFT to be claimed
    rarefractalsNFTClaimed[_rareFractalsTokenID] = 1;
    _mint(_myTokenURI, _poemName, _poemText, _poemAuthor);
  }
  ...
  function _mint(
    ...
    emit TokenBought(newItemId, _poemName, _poemText, _poemAuthor);
