...
contract PartySkull {
    ...
    event CranialBones(uint256[8] cranial_bone_list);
    
    event AuditoryOssicles(uint256[6] auditory_ossicle_list);
    
    event FacialBones(uint256[14] facial_bone_list);
    
    event Skull(uint256[3] skull_list);
    
    function setaddressPartyBones(address address_) public
    {
    ...
    function setaddressPartyStructures(address address_) public
    {
        addressPartyStructures = address_;
    }
    
    function createCranialBones(uint256 LeftParietalTokenId, uint256 RightParietalTokenId, uint256 LeftTemporalTokenId, uint256 RightTemporalTokenId, 
        uint256 FrontalTokenId, uint256 OccipitalTokenId, uint256 EthmoidTokenId, uint256 SphenoidTokenId)
    	public
    	returns (uint256)
    {   
        ...
        uint256[8] memory tokenlist = [LeftParietalTokenId, RightParietalTokenId, LeftTemporalTokenId, RightTemporalTokenId, 
                                            FrontalTokenId, OccipitalTokenId, EthmoidTokenId, SphenoidTokenId];
                                        
        uint8[8] memory modulus_list = [71, 164, 91, 184, 10, 104, 9, 192];
        ...
        uint256 newItemId = partyStructures.createStructure(msg.sender, "Cranial Bones");
        
        emit CranialBones(tokenlist);
        
        return newItemId;
    }
    
    function createAuditoryOssicles(uint256 LeftMalleusTokenId, uint256 RightMalleusTokenId, uint256 LeftIncusTokenId, uint256 RightIncusTokenId, 
        uint256 LeftStapesTokenId, uint256 RightStapesTokenId)
    	public
    	returns (uint256)
    {   
        PartyBones partyBones = PartyBones(addressPartyBones);
        PartyStructures partyStructures = PartyStructures(addressPartyStructures);
        uint256[6] memory tokenlist = [LeftMalleusTokenId, RightMalleusTokenId, LeftIncusTokenId, RightIncusTokenId, LeftStapesTokenId, RightStapesTokenId];
        uint8[6] memory modulus_list = [61, 154, 57, 150, 89, 182];
        ...
        uint256 newItemId = partyStructures.createStructure(msg.sender, "Auditory Ossicles");
        
        emit AuditoryOssicles(tokenlist);
        
        return newItemId;
    }

    function createFacialBones(uint256[] memory MaxillaTokenIds, uint256[] memory ZygomaticTokenIds,
        uint256 MandibleTokenId, uint256[] memory NasalTokenIds, uint256[] memory PalatineTokenIds,
        uint256 LeftInferiorNasalConchaTokenId, uint256 RightInferiorNasalTokenId, uint256 LeftLacrimalTokenId, uint256 RightLacrimalTokenId, uint256 VomerTokenId)
    	public
    	returns (uint256)
    {   
        PartyBones partyBones = PartyBones(addressPartyBones);
        ...
        uint256[14] memory tokenlist = [MaxillaTokenIds[0], MaxillaTokenIds[1], ZygomaticTokenIds[0], ZygomaticTokenIds[1], MandibleTokenId,
                                        NasalTokenIds[0], NasalTokenIds[1], PalatineTokenIds[0], PalatineTokenIds[1],
                                        LeftInferiorNasalConchaTokenId, RightInferiorNasalTokenId, LeftLacrimalTokenId,
                                        RightLacrimalTokenId, VomerTokenId];
                                        
        uint8[14] memory modulus_list = [62, 155, 97, 190, 103, 68, 161, 70, 163, 58, 151, 59, 152, 0];
        ...
        uint256 newItemId = partyStructures.createStructure(msg.sender, "Facial Bones");
        
        emit FacialBones(tokenlist);
        
        return newItemId;
    }
    
    function createSkull(uint256 FacialBonesTokenId, uint256 AuditoryOssiclesTokenId, uint256 CranialBonesTokenId)
    	public
    	returns (uint256)
    {   
        PartyStructures partyStructures = PartyStructures(addressPartyStructures);
        
        uint256[3] memory tokenlist = [FacialBonesTokenId, AuditoryOssiclesTokenId, CranialBonesTokenId];
                                        
        string[3] memory name_list = ["Facial Bones", "Auditory Ossicles", "Cranial Bones"];
        
        for (uint256 i = 0; i < tokenlist.length; i++){
            require( keccak256(bytes(partyStructures.check_structure_ID_name(tokenlist[i]))) == keccak256(bytes(name_list[i])) , "Please check your Token IDs!" );
        ...
        uint256 newItemId = partyStructures.createStructure(msg.sender, "Skull");
        
        emit Skull(tokenlist);
