...
contract iiVault  {
  ...
  constructor (address _token,uint256 _earnLowerlimit) public {
      token = IERC20(_token);
      getName = string(abi.encodePacked("afi:iiVault:", ERC20Detailed(_token).name()));

      earnLowerlimit = _earnLowerlimit*1e18;
      afitoken = IERC20(afi);
      governance = tx.origin;
      controller = 0x03aEc030F5430927948fCb2Cf30f5913999a510f;
  }
  ...
  function withdraw(uint amount) external {
    ...
    function make_profit(uint256 amount) public { 
        ...
        afitoken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function claim() public { 
        ...
        if (out > 0) {
            ...
            if (_depositTime < 1 days){ //deposit in 24h
                ...
                afitoken.safeTransfer(Controller(controller).rewards(), to_team);
                out = actually_out;
            }
            afitoken.safeTransfer(msg.sender, out);
        }
    }
