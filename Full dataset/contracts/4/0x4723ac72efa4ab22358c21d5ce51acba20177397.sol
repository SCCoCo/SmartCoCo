...
contract DAIVault is Ownable {
  ...
  // Can only be called if emergency withdraw is on. (once per month to increase rewards)
  function unstakeDai() public {
    ...
    User storage user = userDetails[_msgSender()];
    require(user._DaiDeposited > 0, "User has no stake");

    uint256 _pendingRewards = user._DaiDeposited.mul(accLunaCorePerShare).div(1e18).sub(user._rewardDebt);

    uint256 beingWithdrawn = user._DaiDeposited;

    user._DaiDeposited = 0;
    user._rewardDebt = 0;

    totalStaked = totalStaked.sub(beingWithdrawn);

    DaiToken.transfer(_msgSender(), beingWithdrawn);
    LunaCoreToken.transfer(_msgSender(), _pendingRewards);
