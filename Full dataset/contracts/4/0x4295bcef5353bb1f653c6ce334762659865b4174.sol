...
contract XCLSV is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10000 * (10**3);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;


    string private _name = "Exclusive Coin";
    string private _symbol = "XCLSV";
    ...
    uint256 private _previousCharityFee = Burnfee;
    ...
    uint256 public _maxTxAmount = 2000 * (10**3);
    uint256 private numTokensSellToAddToLiquidity = 1000 * (10**3);
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
     ...
     //to recieve BNB from panecakeV2Router when swaping
    ...
    function _takeCharity(uint256 tCharity) private {
        uint256 currentRate =  _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[deadWallet] = _rOwned[deadWallet].add(rCharity);
        if(_isExcluded[deadWallet])
            _tOwned[deadWallet] = _tOwned[deadWallet].add(tCharity);
    }
    ...
    function removeAllFee() private {
        if(Burnfee == 0 && _liquidityFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousCharityFee = Burnfee;
