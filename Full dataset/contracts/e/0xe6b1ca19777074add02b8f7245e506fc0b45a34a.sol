...
contract ICOEth is  Ownable, Pausable {
  ...
  // The percent that unlocked after unlocktime. 8%
  uint256 private firstUnlockPercent = 8; 
  ...
  uint256 private eightMonths = 21037944;
  ...
  // -----------------------------------------
   ...
   function unlockedToken(address _user) public view returns (uint256) {
      ...
      if(unlockTime == 0) {
          ...
          if (timePassed < eightMonths) {
            unlocked = user.totalRewardAmount.mul(firstUnlockPercent).div(100);
          }
          else {
            timePassed = timePassed.sub(eightMonths);
