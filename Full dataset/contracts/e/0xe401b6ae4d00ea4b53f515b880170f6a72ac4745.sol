...
abstract contract StrategyMithFarmBase is StrategyStakingRewardsBase {
    ...
    function harvest() public override onlyBenevolent {
        ...
        uint256 _mis = IERC20_1(mis).balanceOf(address(this));
        if (_mis > 0) {
            // 10% is locked up for future gov
            uint256 _keepMIS = _mis.mul(keepMIS).div(keepMISMax);
            IERC20_1(mis).safeTransfer(
                IController(controller).treasury(),
                _keepMIS
            );
        ...
        uint256 _usdt = IERC20_1(usdt).balanceOf(address(this));
        if (_usdt > 0) {
            _swapSushiswapWithPath(usdt_token1_path, _usdt.div(2));
        }

        // Adds in liquidity for USDT/Token
        _usdt = IERC20_1(usdt).balanceOf(address(this));
        uint256 _token1 = IERC20_1(token1).balanceOf(address(this));
        if (_usdt > 0 && _token1 > 0) {
            IERC20_1(usdt).safeApprove(sushiRouter, 0);
            IERC20_1(usdt).safeApprove(sushiRouter, _usdt);

            IERC20_1(token1).safeApprove(sushiRouter, 0);
            IERC20_1(token1).safeApprove(sushiRouter, _token1);
            ...
            // Donates DUST
            IERC20_1(usdt).safeTransfer(
                IController(controller).treasury(),
                IERC20_1(usdt).balanceOf(address(this))
            );
...
////// src/strategies/mith/strategy-mith-mis-usdt-lp.sol
/* pragma solidity ^0.6.7; */
