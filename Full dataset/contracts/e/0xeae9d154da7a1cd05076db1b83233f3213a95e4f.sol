...
library StringUtils {
  ...
  function atoi(string a, uint8 base) internal pure returns (uint256 i) {
    ...
    for(uint256 p = 0; p < buf.length; p++) {
      ...
      // if digit is greater then 10 每 mind the gap
...
contract GemERC721 is AccessControl, ERC165 {
  ...
  string public constant name = "GEM 每 CryptoMiner World";
  ...
  /// @dev Consists of a single bit at position 1 每 binary 1
   ...
   * ERC721 interface extension 每 exists(uint256)
   ...
   * @dev Gets the gem's properties 每 color, level and
   ...
   *      approved 每 an address explicitly approved earlier by
  ...
  function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes _data) public {
    ...
    if (AddressUtils.isContract(_to)) {
      ...
      // if `_to` is a contract 每 execute onERC721Received
