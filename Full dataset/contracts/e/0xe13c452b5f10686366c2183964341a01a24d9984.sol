...
contract Strategy is CurveVoterProxy {
    ...
    function setIsUniswapV3(bool _isUniswapV3) external onlyAuthorized {
        isUniswapV3 = _isUniswapV3;
    }

    function _approveAll() internal {
        IERC20(crv).approve(sushiswap, 0);
        IERC20(crv).approve(sushiswap, uint(-1));
        IERC20(crv).approve(uniswap, 0);
        IERC20(crv).approve(uniswap, uint(-1));
        IERC20(lqty).approve(uniswapv3, 0);
        IERC20(lqty).approve(uniswapv3, uint(-1));
        IERC20(lqty).approve(uniswap, 0);
        IERC20(lqty).approve(uniswap, uint(-1));
        IERC20(dai).safeApprove(curve, 0);
        IERC20(dai).safeApprove(curve, uint(-1));
        IERC20(usdc).safeApprove(curve, 0);
        IERC20(usdc).safeApprove(curve, uint(-1));
        IERC20(usdt).safeApprove(curve, 0);
        IERC20(usdt).safeApprove(curve, uint(-1));
    }
