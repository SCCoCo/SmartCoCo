...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to VENI Finance.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // VENI Finance must mint EXACTLY the same amount of VENI Finance LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// VENIAdmin is the master of VENI. He can make VENI and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once VENI is sufficiently
...
contract VENIAdmin is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of VENIs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accVENIPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accVENIPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Block number when bonus VENI period ends.
    uint256 public bonusEndBlock;
    // VENI tokens created per block.
    ...
    // The block number when VENI mining starts.
    ...
    // View function to see pending VENIs on frontend.
    ...
    // Safe veni transfer function, just in case if rounding error causes pool to not have enough VENIs.
    ...
    // Deposit single tokens to VENIAdmin for VENI allocation.
    ...
    // Withdraw single tokens from VENIAdmin.
