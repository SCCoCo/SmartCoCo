...
/**
 * @title Defilend's CErc20 Contract
 * @notice CTokens which wrap an EIP-20 underlying
 * @author DeFIL
 */
contract CErc20 is CToken, CErc20Interface, Distributor {
    /**
    ...
    function accruedStored(address account) external view returns (uint) {
        return historicalReserveKeeperAccrued[account];
    }

    function accrue() external returns (uint) {
        require(accrueInterest() == uint(Error.NO_ERROR), "accrue interest failed");
        return historicalReserveKeeperAccrued[msg.sender];
    }

    function claim(address receiver, uint amount) external returns (uint) {
        require(accrueInterest() == uint(Error.NO_ERROR), "accrue interest failed");
        require(_reduceReservesFresh(receiver, amount) == uint(Error.NO_ERROR), "reduce reserves failed");
        return amount;
    }
...
// Copied from compound/CErc20Delegate
/**
 * @title Defilend's CErc20Delegate Contract
