...
contract yUSDTvault is VaultWrapper(0xdAC17F958D2ee523a2206206994597C13D831ec7), IGovernanceAddressRecipient {
    ...
    function remainingReward() public view returns (uint256) {
        return YFFS.balanceOf(address(this));
    }

    function stop() public onlyGovernanceAddress {
        require(vaultOpen, "Vault is not opened.");
        YFFS.safeTransfer(address(0x0000000000000000000000000000000000000001), remainingReward());
        vaultOpen = false;
        initReward = 0;
        rewardRate = 0;
        emit StopVault(block.timestamp);
    }
    ...
    function start() public onlyGovernanceAddress {
        require(!vaultOpen, "Vault is opened.");
    ...
    function depositReward(uint256 amount) public onlyGovernanceAddress {
        ...
        YFFS.safeTransferFrom(msg.sender, address(this), amount);
        initReward = amount;
        rewardRate = initReward.div(DURATION);
        emit DepositVaultReward(amount);
    }
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        ...
        if (reward > 0) {
            ...
            YFFS.safeTransfer(msg.sender, reward);
