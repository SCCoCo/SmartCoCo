...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /* Origin address */
    address internal constant ORIGIN_ADDR = 0xC7996F2f9D1ff26B3ED4B8CD001DB06C6845A541;

    /* Flush address */
    address payable internal constant FLUSH_ADDR = 0x9e958Ec9C92cf5029291246D3319A6a3E2B5e21f;
    ...
    uint256 private constant HEARTS_PER_OCT = 10 ** uint256(decimals); // 1e8
    uint256 private constant OCT_PER_BTC = 1e4;
    uint256 private constant SATOSHIS_PER_BTC = 1e8;
    uint256 internal constant HEARTS_PER_SATOSHI = HEARTS_PER_OCT / SATOSHIS_PER_BTC * OCT_PER_BTC;

    /* Time of contract launch (2019-12-08T00:00:00Z) */
    uint256 internal constant LAUNCH_TIME = 1575763200;
    ...
    /* Seed for WAAS Lobby */
    uint256 internal constant WAAS_LOBBY_SEED_OCT = 1e9;
    uint256 internal constant WAAS_LOBBY_SEED_HEARTS = WAAS_LOBBY_SEED_OCT * HEARTS_PER_OCT;
    ...
    uint256 private constant BPB_MAX_OCT = 150 * 1e6;
    uint256 internal constant BPB_MAX_HEARTS = BPB_MAX_OCT * HEARTS_PER_OCT;
    ...
    uint8 internal constant ETH_ADDRESS_OCT_LEN = ETH_ADDRESS_BYTE_LEN * 2;

    uint8 internal constant CLAIM_PARAM_HASH_BYTE_LEN = 12;
    uint8 internal constant CLAIM_PARAM_HASH_OCT_LEN = CLAIM_PARAM_HASH_BYTE_LEN * 2;

    uint8 internal constant BITCOIN_SIG_PREFIX_LEN = 24;
    bytes24 internal constant BITCOIN_SIG_PREFIX_STR = "Bitcoin Signed Message:\n";

    bytes internal constant STD_CLAIM_PREFIX_STR = "Claim_OCT_to_0x";
    bytes internal constant OLD_CLAIM_PREFIX_STR = "Claim_BitcoinOCT_to_0x";

    bytes16 internal constant OCT_DIGITS = "0123456789abcdef";
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "OCT: Too early");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "OCT: range invalid");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        internal
    {
        ...
        require(newStakeShares != 0, "OCT: newStakedHearts must be at least minimum shareRate");
...
contract UTXOClaimValidation is StakeableToken {
        ...
        returns (bytes memory)
    {
        ...
        if (claimParamHash == 0) {
            return abi.encodePacked(
                ...
                uint8(prefixStr.length) + ETH_ADDRESS_OCT_LEN,
                prefixStr,
                addrStr
            );
        }

        bytes memory claimParamHashStr = new bytes(CLAIM_PARAM_HASH_OCT_LEN);
        ...
        return abi.encodePacked(
            BITCOIN_SIG_PREFIX_LEN,
            BITCOIN_SIG_PREFIX_STR,
            uint8(prefixStr.length) + ETH_ADDRESS_OCT_LEN + 1 + CLAIM_PARAM_HASH_OCT_LEN,
            prefixStr,
            addrStr,
            "_",
            claimParamHashStr
    ...
    {
        addrStr = new bytes(ETH_ADDRESS_OCT_LEN);
        ...
        pure
    {
        ...
        for (uint256 i = 0; i < dataLen; i++) {
            ...
            hexStr[offset++] = OCT_DIGITS[b >> 4];
            hexStr[offset++] = OCT_DIGITS[b & 0x0f];
        }
    }
