...
dailyHoldBalance,balanceともに初期値0なのでbonusは0
...
/*
 ...
 x0, y0, ratePlus, rateMinusは

 上位0~127bit目は掛け算のbuffer。常に0で埋まっていること。

 128~159bit目は整数部分, 160~255bit目は小数部分を表す。[0, 2^32)を1/2^96刻みで扱える。

 Token balanceは2^128 =~ 3.4*10^38 を超えてはならない。
