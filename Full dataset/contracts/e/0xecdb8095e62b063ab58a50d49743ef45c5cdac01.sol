...
// File contracts/Private-Clients/RFISupportingCharity.sol
...
contract RFISupportingCharity is Context, IERC20, Ownable {
    ...
    event CharityAddressChanged(address);
    ...
    constructor(
        ...
        emit CharityAddressChanged(charityAddress_);
    ...
    function excludeAccountFromRewards(address account) external onlyOwner() {
        ...
        require(_excludedFromRewards.length < 15, "Too many addresses excluded");
    ...
    function excludeAccountFromFee(address account) external onlyOwner() {
        require(_isExcludedFromFee[account] == false, "Account already excluded from fee");

        _isExcludedFromFee[account] = true;
    }

    function includeAccountInFee(address account) external onlyOwner() {
        require(_isExcludedFromFee[account] == true, "Account already included in fee");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (takeFee){
            amount = _transferHook(sender, amount);
        }
    ...
    function setCharityAddress(address charityAddress_) internal {
        charityAddress = charityAddress_;
        emit CharityAddressChanged(charityAddress);
    }
    ...
    function LowLevelTransfer(address sender, address recipient, uint256 amount) internal{
    ...
    function _transferHook(address sender, uint256 amount) internal returns (uint256){
        uint256 tCharityAmount = amount.mul(charityFee).div(100);
        LowLevelTransfer(sender, charityAddress, tCharityAmount);

        uint256 leftOverAmountToTransfer = amount.sub(tCharityAmount);
        
        return leftOverAmountToTransfer;
