...
contract FilstMarketStorage {
    // address of the eFIL token
    address public efilAddress;

    // address of the FilstPool
    address public filstPoolAddress;

    // address of the eFIL market
    address public efilMarketAddress;

    // Stores the cash
    uint internal cash;

    // The prior accrued amount of this address in FilstPool
    uint public filstPoolAccruedAmount;

    struct AccountState {
        // The last accrued index of member
        uint accruedIndex;
        /// @notice The accrued but not yet transferred to member
        uint accruedAmount;
    }

    // The global accrued index for eFIL
    uint public eFilGlobalAccruedIndex;

    // The accrued state for each account
    mapping(address => AccountState) public eFilAccountStates;
}

// Modified from compound/CErc20
contract FilstPool is Distributor {
    // deposit FILST, returns actually deposit amount
    function deposit(uint amount) external returns (uint);

    // withdraw FILST, returns actually withdraw amount
    function withdraw(uint amount) external returns (uint);
}

/**
 * @title FILST Market Contract
 */
contract FilstMarketCErc20 is CollateralOnlyCErc20, FilstMarketStorage {
    // The initial accrual index
    uint internal constant initialAccruedIndex = 1e36;

    /*** Events ***/
    // Emitted when accrued
    event AccruedEFIL(uint amount, uint eFilGlobalAccruedIndex);

    // Emitted when distribute eFil to a account
    event DistributedEFIL(address account, uint amount, uint accruedIndex);

    // Emitted when account claims eFIL
    event ClaimedEFIL(address account, uint amount);

    // Emitted when account repay borrow in eFIL market with accrued eFIL
    event RepayBorrowBehalfInEfilMarket(address account, uint amount);
     ...
     * @param initialExchangeRateMantissa_ The initial exchange rate, scaled by 1e18
     */
    function initialize(address filstAddress_,
                        ComptrollerInterface comptroller_,
                        InterestRateModel interestRateModel_,
                        uint initialExchangeRateMantissa_,
                        string memory name_,
                        ...
                        address filstPoolAddress_,
                        address efilMarketAddress_) public {
        // CToken initialize does the bulk of the work
        super.initialize(filstAddress_, comptroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_);

        // set addresses
        efilAddress = efilAddress_;
        filstPoolAddress = filstPoolAddress_;
        efilMarketAddress = efilMarketAddress_;

        // approve FILST for FilstPool
        EIP20Interface FILST = EIP20Interface(underlying);
        FILST.approve(filstPoolAddress, uint(-1));

        // approve eFIL for eFIL market
        EIP20Interface eFIL = EIP20Interface(efilAddress);
        eFIL.approve(efilMarketAddress, uint(-1));

        // set eFilGlobalAccruedIndex
        eFilGlobalAccruedIndex = initialAccruedIndex;
    }

    /*** override ***/

    // override transferTokens
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        accrueEfil();
        distributeEfilInternal(src);
        distributeEfilInternal(dst);
    ...
    function seize(address liquidator, address borrower, uint seizeTokens) external nonReentrant returns (uint) {
        accrueEfil();
        distributeEfilInternal(liquidator);
        distributeEfilInternal(borrower);
    ...
    function repayBorrowBehalfInEfilMarket(uint repayAmount) external {
        address account = msg.sender;
        // keep fresh
        accrueEfil();
        distributeEfilInternal(account);

        // reference storage
        AccountState storage state = eFilAccountStates[account];

        /* If repayAmount == 0, repayAmount = state.accruedAmount */
        uint actualRepayAmount = repayAmount;
        if (actualRepayAmount == 0) {
            actualRepayAmount = state.accruedAmount;
        }

        // get fresh accountBorrows
        uint accountBorrows = CToken(efilMarketAddress).borrowBalanceCurrent(account);
        if (actualRepayAmount > accountBorrows) {
            actualRepayAmount = accountBorrows;
        }

        // check balance
        require(actualRepayAmount <= state.accruedAmount, "insufficent value");

        // makes FilstPool transfer eFIL to the this address
        require(FilstPool(filstPoolAddress).claim(address(this), actualRepayAmount) == actualRepayAmount, "FilstPool.claim: amount mismatch");
        require(CErc20Interface(efilMarketAddress).repayBorrowBehalf(account, actualRepayAmount) == uint(Error.NO_ERROR), "repayBorrowBehalf falied");

        // update storage
        state.accruedAmount = sub_(state.accruedAmount, actualRepayAmount);
        filstPoolAccruedAmount = sub_(filstPoolAccruedAmount, actualRepayAmount);

        emit RepayBorrowBehalfInEfilMarket(account, actualRepayAmount);
    }

    function claimEfil() external {
        address account = msg.sender;
        // keep fresh
        accrueEfil();
        distributeEfilInternal(account);

        // only when there is no balance or no debts
        if (balanceOf(account) != 0) {
            address[] memory assets = comptroller.getAccountAssets(account);
            bool hasDebts = false;
            for (uint i = 0; i < assets.length; i++) {
                if (assets[i] != address(this) && CToken(assets[i]).borrowBalanceStored(account) > 0) {
                    hasDebts = true;
                    break;
                }
            }
            require(!hasDebts, "claimEfil: still has debts");
        }

        AccountState storage state = eFilAccountStates[account];
        uint claimAmount = state.accruedAmount;

        // makes FilstPool transfer eFIL to the account
        require(FilstPool(filstPoolAddress).claim(account, claimAmount) == claimAmount, "claimEfil: amount mismatch");

        // update
        state.accruedAmount = sub_(state.accruedAmount, claimAmount);
        filstPoolAccruedAmount = sub_(filstPoolAccruedAmount, claimAmount);

        emit ClaimedEFIL(account, claimAmount);
    }

    // Return the accrued amount of account based on stored data
    function accruedEfilStored(address account) external view returns(uint) {
        uint storedGlobalAccruedIndex;
        if (totalSupply == 0) {
            storedGlobalAccruedIndex = eFilGlobalAccruedIndex;
        } else {
            uint newAccruedAmount = FilstPool(filstPoolAddress).accruedStored(address(this));
            uint delta = sub_(newAccruedAmount, filstPoolAccruedAmount);

            Double memory ratio = fraction(delta, totalSupply);
            Double memory doubleAccruedIndex = add_(Double({mantissa: eFilGlobalAccruedIndex}), ratio);
            storedGlobalAccruedIndex = doubleAccruedIndex.mantissa;
        }

        (, uint instantAccountAccruedAmount) = accruedEfilStoredInternal(account, storedGlobalAccruedIndex);
        return instantAccountAccruedAmount;
    }

    function accruedEfilStoredInternal(address account, uint withGlobalAccruedIndex) internal view returns(uint, uint) {
        AccountState memory state = eFilAccountStates[account];
        ...
        uint delta = mul_(accountBalance, deltaIndex);

        return (delta, add_(state.accruedAmount, delta));
    }

    function accrueEfil() internal {
        if (totalSupply == 0) {
            return;
        }

        uint newAccruedAmount = FilstPool(filstPoolAddress).accrue();
        uint delta = sub_(newAccruedAmount, filstPoolAccruedAmount);

        Double memory ratio = fraction(delta, totalSupply);
        Double memory doubleAccruedIndex = add_(Double({mantissa: eFilGlobalAccruedIndex}), ratio);

        // update eFilGlobalAccruedIndex
        eFilGlobalAccruedIndex = doubleAccruedIndex.mantissa;
        filstPoolAccruedAmount = newAccruedAmount;

        emit AccruedEFIL(delta, doubleAccruedIndex.mantissa);
    }

    function distributeEfilInternal(address account) internal {
        (uint delta, uint instantAccountAccruedAmount) = accruedEfilStoredInternal(account, eFilGlobalAccruedIndex);

        AccountState storage state = eFilAccountStates[account];
        state.accruedIndex = eFilGlobalAccruedIndex;
        state.accruedAmount = instantAccountAccruedAmount;

        emit DistributedEFIL(account, delta, eFilGlobalAccruedIndex);
    }

    function doTransferIn(address account, uint amount) internal returns (uint) {
        accrueEfil();
        distributeEfilInternal(account);

        uint actualAmount = super.doTransferIn(account, amount);

        // deposit to FilstPool
        require(FilstPool(filstPoolAddress).deposit(actualAmount) == actualAmount, "FilstPool.deposit: amount mismatch");

        cash = add_(cash, actualAmount);

        return actualAmount;
    }

    function doTransferOut(address account, uint amount) internal {
        accrueEfil();
        distributeEfilInternal(account);

        // withdraw from FilstPool
        require(FilstPool(filstPoolAddress).withdraw(amount) == amount, "FilstPool.withdraw: amount mismatch");

        super.doTransferOut(account, amount);

        cash = sub_(cash, amount);
    }
...
/**
 * @title Defilend's FilstMarketCErc20Delegate Contract
 * @author DeFIL
 */
contract FilstMarketCErc20Delegate is FilstMarketCErc20, CDelegateInterface {
    /**
