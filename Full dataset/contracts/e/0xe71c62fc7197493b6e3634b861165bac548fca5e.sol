...
// File: contracts/DogenMasterDog.sol
...
contract DogenMasterDog is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 reward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        dogenToken.mint(devAddr, reward.div(200).mul(7)); // 3.5%
        dogenToken.mint(founderAddr, reward.div(200).mul(7)); // 3.5%
        dogenToken.mint(communityAddr, reward.div(25).mul(2)); // 8%
        dogenToken.mint(address(this), reward);
        pool.accRewardPerShare = pool.accRewardPerShare.add(reward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterDog for reward allocation.
    ...
    // Withdraw LP tokens from MasterDog.
    ...
    // Update founderAddr address by the previous founderAddr.
    function founder(address _founderAddr) public {
        require(msg.sender == devAddr, "dev: wut?");
        founderAddr = _founderAddr;
    }

    // Update communityAddr address by the previous communityAddr.
