...
contract JUNKToken is ERC20("Junkyard Factory Token", "JUNK"), Ownable {
...
contract JunkyardProtocol is Ownable {
    using SafeMath for uint256;
    ...
    // The TOKEN!
    JUNKToken public swapToken;
    // Block number when bonus token period ends.
    uint256 public bonusEndBlock = 8621080;
    // Token amount created per block.
    uint256 public tokenPerBlock;
    // Bonus muliplier for early swapToken makers.
    ...
    uint256 public halvePeriod = 2880;
    uint256 public lastHalveBlock;
    uint256 public minimumTokenPerBlock = 0.00625 ether;
    ...
    event Halve(uint256 newTokenPerBlock, uint256 nextHalveBlockNumber);

    constructor(
        JUNKToken _swapToken,
        uint256 _startBlock
    ) public {
        swapToken = _swapToken;
        tokenPerBlock = 0.05 ether;
        startBlock = _startBlock;
        lastHalveBlock = _startBlock + 0;
    }

    function doHalvingCheck(bool _withUpdate) public {
        if (tokenPerBlock <= minimumTokenPerBlock) {
            return;
        }
        ...
        uint256 newTokenPerBlock = tokenPerBlock.div(2);
        if (newTokenPerBlock >= minimumTokenPerBlock) {
            tokenPerBlock = newTokenPerBlock;
            lastHalveBlock = block.number;
            emit Halve(newTokenPerBlock, block.number + halvePeriod);
    ...
    // Safe swapToken transfer function, just in case if rounding error causes pool to not have enough tokens.
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBal = swapToken.balanceOf(address(this));
        if (_amount > tokenBal) {
            swapToken.transfer(_to, tokenBal);
        } else {
