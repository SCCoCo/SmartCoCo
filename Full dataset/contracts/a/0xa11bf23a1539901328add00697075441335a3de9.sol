# KRIA - An ultra-deflationary token made for traders and inflation arbitrators
#
# KRIA has rules based on turns. It automatically burns, mints, airdrops
# and features a dynamic supply range between 15,000 KRIA and 0.15 KRIA
#
# 10% of each transaction will be burned forever
# 5% of every transaction goes to an airdrop address.
# 1% of every transaction goes to the owner, marketing and liquidity incrementation.
# Website: https://www.kriadefi.com
# Telegram: https://t.me/kria_defi
#
...
def __init__(_name: String[64], _symbol: String[32], _decimals: uint256, _supply: uint256, _min_supply: uint256, _max_supply: uint256):
    ...
    self.mint_pct = convert(0.1 * deciCalc, uint256)
    self.burn_pct = convert(0.1 * deciCalc, uint256)
    self.airdrop_pct = convert(0.05 * deciCalc, uint256)
    self.treasury_pct = convert(0.01 * deciCalc, uint256)
...
def transfer(_to : address, _value : uint256) -> bool:
    ...
    if msg.sender != self.owner:
        if self.botThrottling == True:
            if self.tx_n < 200:
                assert _value < 30 * 10 ** self.decimals, "Maximum amount allowed is 30 KRIA until the 200th transaction."
