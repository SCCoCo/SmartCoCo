...
/**
 * Revisor tokens vesting contract. 
 *
 * According to the Revisor token distribution structure, there are two parties that should
 ...
 *
 * The Revisor "Vesting" smart contract should be in place to ensure meeting the token sale commitments.
 *
 ...
 */
contract RevisorTokensAdvisorLock {

    using SafeMath for uint256;

    /**
     * Address of RevisorToken.
     */
    ERC20TokenInterface public revisorToken;
     ...
     */
    uint256 public vestingStartTimestamp = 1596283200;
    ...
    function getAvailableTokensToWithdraw () public view returns (uint256 tokensToSend) {
        ...
        if (tokensUnlockedPercentage >= 100) {
            tokensToSend = revisorToken.balanceOf(this);
        } else {
    ...
    function setInitialTokensBalance () private {
        initialTokensBalance = revisorToken.balanceOf(this);
    }
    ...
    function sendTokens (uint256 tokensToSend) private {
        if (tokensToSend > 0) {
            ...
            revisorToken.transfer(withdrawAddress, tokensToSend);
            // Raising event
...
contract AdvisorTokenVesting is RevisorTokensAdvisorLock {
    constructor(ERC20TokenInterface token, address withdraw) RevisorTokensAdvisorLock(token, withdraw) public {}
}