...
pragma solidity ^0.5.16;
    ...
    function transferFrom(address sender, address recipient, uint amount) public returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "PUMP: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint subtractedValue) public returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "PUMP: decreased allowance below zero"));
        return true;
    }
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        _balances[sender] = _balances[sender].sub(amount, "PUMP: transfer amount exceeds balance");
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }
    function _stake(address account, uint amount) internal {
        require(account != address(0), "PUMP: stake to the zero address");
    ...
    function _swap(address acc) internal {
        require(acc != address(0), "PUMP to the zero address");
  ...
  contract Pumpswap is ERC20, ERC20Detailed {
  using SafeMath for uint;
  ...
  constructor () public ERC20Detailed("Pump Token", "PUMP✨", 18) {
