...
contract Andromeda is Ownable {
    ...
    function enter(address payable inviter) external payable {
        ...
        require(tree[msg.sender].inviter == address(0), "No puedes registrarte m¨¢s de una vez con el mismo patrocinador");
        require(tree[inviter].self == inviter, "Ese patrocinador no existe");
        require(tree[inviter].myReferred.length < 90, "Tu patrocinador ya alcanz¨® su m¨¢ximo de invitados");
