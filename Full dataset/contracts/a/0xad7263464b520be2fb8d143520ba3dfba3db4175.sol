...
contract ZEUX {
	...
	uint256 constant private Total_ZUC = 75e25;
	uint256 constant private Burn_Ratio = 4;
	uint256 constant private Burn_Threshold = 5;     
	uint256 constant private Freeze_Size = 1e20; 
	
	string constant public name = "Zeux Finance";
	string constant public symbol = "ZUC";
	...
	constructor() public {
		...
		info.totalSupply = Total_ZUC;
		info.users[msg.sender].balance = Total_ZUC;
		emit Transfer(address(0x0), msg.sender, Total_ZUC);
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		if (totalSupply() - _burnedAmount < Total_ZUC * Burn_Threshold / 100 || isWhitelisted(_from)) {
			_burnedAmount = 0;
		}
