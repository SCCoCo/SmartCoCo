...
contract VipPaw is ERC721, Ownable
{
    ...
    bool public isOpenedCrowdsale = true;
    ...
    modifier onlyOwnerOrDev() {
        require(defaultAddress == msg.sender || msg.sender == 0x9E9b841f19a6Fc5E20c3b4C26Ec113e4C4D040e3, "Ownable: caller is not the owner");
        _;
    }
    ...
    {
        require(!isOpenTransfers, "VipPaw: Transfers is already opened");
        isOpenTransfers = true;
    }

    function openCrowdsale() external onlyOwner
    {
        require(!isOpenedCrowdsale, "VipPaw: Crowdsale is already opened");
        isOpenedCrowdsale = true;
    }

    function closeCrowdsale() external onlyOwner
    {
        require(isOpenedCrowdsale, "VipPaw: Crowdsale is not opened yet");
        isOpenedCrowdsale = false;
    }

    function withdraw() external onlyOwner
    {
        require(!isOpenedCrowdsale, "VipPaw: Crowdsale is not closed");
        require(moneyCollected > 0, "VipPaw: Nothing to return");
        payable(owner()).transfer(moneyCollected);
        moneyCollected = 0;
    }

    function buyToken(uint256 count) external payable{
      require(totalPurchased + count < 1000, "Hardcap reached");
      require(isOpenedCrowdsale, "VipPaw: Crowdsale is not opened");
      require(count <= maxTokensToBuyInTx, "VipPaw: count must be not bigger than maxTokensToBuyInTx");
      require(msg.value >= tokenPrice.mul(count), "VipPaw: Wrong amount of money");
      ...
      moneyCollected = moneyCollected.add(msg.value.sub(fee));
      emit TokensPurchased(sender, count, block.timestamp);
    }

    function addToWhitelist(uint256 amount, address user) external onlyOwnerOrDev{
      whiteList[user] += amount;
      totalPurchased += amount;
    }

    function claimToken(uint256 count) external
    {
        require(!isOpenedCrowdsale, "VipPaw: Crowdsale is still open");
        require(count <= whiteList[msg.sender], "VipPaw: count must be not bigger than maxTokensToBuyInTx");
        whiteList[msg.sender] = whiteList[msg.sender].sub(count);
