...
contract Laxative is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    PaperToken public paper;
    address public paperLP;
    ...
    constructor(PaperToken _paper, address _paperLpToken, address _farmContract, address _WETH, address _developers) public {
        paper = _paper;
    ...
    /*
    - LpBalance     PAPER reward    Total reward
    - 9 ETH         100 PAPER       100 PAPER
    - 18 ETH        200 PAPER;      300 PAPER
    - 54 ETH        300 PAPER;      600 PAPER
    - 90 ETH        500 PAPER;      1 100 PAPER
    - 180 ETH       1 000 PAPER;    2 100 PAPER
    - 900 ETH       2 000 PAPER;    4 100 PAPER
    - 1 800 ETH     3 000 PAPER.    7 100 PAPER
    */
    function getBonus(uint256 LpBalance) public pure returns(uint256) {
        if (LpBalance >= 1800e18) {
            return 7100e18;
        }
        if (LpBalance >= 900e18) {
            return 4100e18;
    ...
    function getLpBalance() public view returns(uint256 amount) {
        return IERC20(WETH).balanceOf(paperLP).mul(2);
    }
    
    function release() public returns(uint256 bonus) {
        bonus = getBonus(getLpBalance());
        require(bonus > released, "Nothing to release");
        bonus = bonus.sub(released);

        paper.mintPaper(farmContract, bonus);
        paper.mintPaper(developers, bonus.div(10));
        released = released.add(bonus);

        emit NewRelease(bonus);
    }
}