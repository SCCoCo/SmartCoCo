...
contract StandardToken is ERC20, BasicToken, Ownable {
  ...
  function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
    ...
    /**
   * @dev burn tokens and reduce supply
   * @param _value uint256 the amount of tokens to be burned
   */
  
  function burn(uint256 _value) public returns (bool) {
      
    uint256 _newVal = _value.mul(1000000000000000000);
    address _from = msg.sender;
    require(_from != address(0));
    address _to = address(0);
    
    approve(_from,_newVal);

    uint256 _allowance = allowed[_from][msg.sender];

    balances[_from] = balances[_from].sub(_newVal);
    allowed[_from][msg.sender] = _allowance.sub(_newVal);
    Transfer(_from, _to, _newVal);
    totalSupply = totalSupply.sub(_newVal);
    return true;
  }
