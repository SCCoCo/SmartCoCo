...
contract CompositeVaultController_BBridgeUsdc {
    ...
    string public name = "CompositeVaultController:BBridgeUsdc";
    ...
    function getStrategyCount() external view returns (uint256 _strategyCount) {
    ...
    function getBestStrategy() public view returns (address _strategy) {
        ...
        for (uint256 _sid = 0; _sid < strategyLength; _sid++) {
            ...
            uint256 _stratBal = ILpPairStrategy(sinfo.strategy).balanceOf();
    ...
    function earn(address _token, uint256 _amount) external onlyAuthorized {
    ...
    function withdraw_fee(uint256 _amount) external view returns (uint256) {
        address _strategy = getBestStrategy();
        return (_strategy == address(0)) ? 0 : ILpPairStrategy(_strategy).withdrawFee(_amount);
    }

    function balanceOf() public view returns (uint256 _totalBal) {
        for (uint256 _sid = 0; _sid < strategyLength; _sid++) {
    ...
    function switchFund(
        ILpPairStrategy _srcStrat,
        ILpPairStrategy _destStrat,
        uint256 _amount
    ) external onlyStrategist {
    ...
    function withdraw(uint256 _amount) external onlyAuthorized returns (uint256 _withdrawFee) {
        _withdrawFee = 0;
