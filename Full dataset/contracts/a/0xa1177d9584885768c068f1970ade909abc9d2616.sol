...
contract AcidDragons is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant ACIDDRAGONS_PUBLIC = 4000;
    uint256 public constant ACIDDRAGONS_MAX = ACIDDRAGONS_PUBLIC;
    ...
    Counters.Counter private _publicACIDDRAGONS;

    constructor() ERC721("ACIDDRAGONS", "LSDRAGON") {}
        ...
        onlyOwner
    {
        ...
        require(
            _publicACIDDRAGONS.current() < 1000,
            "Purchase would exceed ACIDDRAGONS_PUBLIC"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicACIDDRAGONS.current();

            if (_publicACIDDRAGONS.current() < ACIDDRAGONS_PUBLIC) {
                _publicACIDDRAGONS.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicACIDDRAGONS.current() < ACIDDRAGONS_PUBLIC,
            "Purchase would exceed ACIDDRAGONS_PUBLIC"
        );
        require(
